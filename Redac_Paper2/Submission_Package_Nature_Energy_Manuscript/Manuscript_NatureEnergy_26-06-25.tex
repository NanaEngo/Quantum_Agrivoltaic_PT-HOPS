...
Third, the trade-off between SERS diagnostic sensitivity and transport efficiency is captured by a composite figure of merit $\mathrm{FOM} = \Phi_{\mathrm{FT}} \times \mathrm{EF}_{\mathrm{SERS}}$. The $V_{\mathrm{mode}} = \SI{0.8}{\nm\cubed}$ configuration at \SI{295}{\kelvin} achieves the highest FOM (7.68), closely followed by the \SI{77}{\kelvin} configuration (7.41). Notably, the FOM at $V = \SI{0.8}{\nm\cubed}$ is dominated by the larger SERS enhancement factor ($10^2$), while the \SI{77}{\kelvin}$ case compensates for its lower enhancement with a superior transport yield. Both paths produce comparable FOM values, suggesting that the operational choice between cryogenic-sensitivity and room-temperature-sensitivity regimes can be guided by field-specific requirements without a fundamental performance penalty.

All NPoM-on runs exhibit perfect trace conservation ($\Tr\rho = 1.0000 \pm 0.0000$), validating the numerical stability of the PT-HOPS/SBD propagator across $n=100$ disorder realizations at both temperatures. The baseline NPoM-off run is shown for completeness but represents a partial trajectory ($n=2$, \SI{20}{\fs} total simulation time) and should not be compared quantitatively with the converged NPoM-on data.
