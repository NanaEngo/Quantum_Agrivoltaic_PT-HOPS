...
if config_path.exists():
    ...
    print(f"  - Max Hierarchy: {config['simulation_params']['L_max']}")
...
agrivoltaic_model = AgrivoltaicCouplingModel(
    fmo_hamiltonian=H_fmo, temperature=DEFAULT_TEMPERATURE
)
...
try:
    ...
    print(
        f"  - Simple evaluation - PCE: {eval_result['pce']:.4f}, ETR: {eval_result['etr']:.4f}"
    )
...
result_b = eco_analyzer.evaluate_material_sustainability(
    ...
    electron_densities={
        ...
        "n_plus_1": np.array(
            [0.09, 0.11, 0.09, 0.14, 0.11, 0.13, 0.10, 0.15, 0.09, 0.17]
        ),
        "n_minus_1": np.array(
            [0.11, 0.13, 0.11, 0.16, 0.13, 0.15, 0.12, 0.17, 0.11, 0.19]
        ),
    },
    molecular_weight=2000.0,
    bde=310.0,
...
try:
    ...
    print("✓ LCA completed:")
    print(
        f"  - Carbon footprint: {lca_results['carbon_footprint_gco2eq_per_kwh']:.1f} gCO2eq/kWh"
    )
    print(
        f"  - Energy payback time: {lca_results['energy_payback_time_years']:.2f} years"
    )
    print(f"  - EROI: {lca_results['eroi']:.1f}")
    print(
        f"  - Total carbon emissions: {lca_results['total_carbon_kg_co2eq']:.0f} kg CO2-eq"
    )
...
try:
    ...
    print(
        f"  - Revenue per hectare: ${te_results['total_revenue_yr_usd_per_ha']:,.2f}/yr"
    )
except KeyboardInterrupt:
    raise
except (RuntimeError, ValueError, TypeError, OSError) as e:
...
if "t_axis" in results and "populations" in results:
    ...
    quantum_metrics = {
        ...
        if k not in ["t_axis", "populations", "coherences"]
    }
...
if "global_indices" in material_result:
    eco_data["chemical_potential"] = material_result["global_indices"].get(
        "chemical_potential", 0
    )
    eco_data["chemical_hardness"] = material_result["global_indices"].get(
        "chemical_hardness", 0
    )
    eco_data["electrophilicity"] = material_result["global_indices"].get(
        "electrophilicity", 0
    )
...
try:
    optimal_transmission = opt_results.get(
        "optimal_transmission", np.ones_like(lambda_range) * 0.5
    )
    spectral_data = {
...
try:
    fig_path = fig_generator.plot_spectral_optimization(
        opt_results, solar_spectrum=solar_spectrum
    )
...
temperatures = (
    273 + 20 + 10 * np.sin(2 * np.pi * time_days / 365)
)  # Annual temperature variation
humidity_values = 0.5 + 0.2 * np.sin(
    2 * np.pi * time_days / 365 + np.pi / 4
)  # Humidity variation
wind_speeds = 3.0 + 2.0 * np.random.random(
    len(time_days)
)  # Random wind speeds with mean
...
if "optimal_pce" in locals() or "opt_results" in locals():
    opt_pce = (
        opt_results.get("optimal_pce", 0.15) if "opt_results" in locals() else 0.15
    )
    opt_etr = (
        opt_results.get("optimal_etr", 0.85) if "opt_results" in locals() else 0.85
    )
