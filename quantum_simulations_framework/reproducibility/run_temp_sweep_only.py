...
def parse_args():
    ...
    parser.add_argument(
        "--parallel",
        action="store_true",
        help="Enable hardware-aware parallel execution",
    )
    parser.add_argument(
        "--n_jobs",
        type=int,
        default=None,
        help="Force a specific number of parallel workers",
    )
    parser.add_argument(
        "--no-resume",
        action="store_true",
        help="Disable resume logic (restart from scratch)",
    )
...
def run_sweep_with_resume(cfg, H, time_points):
    ...
    new_log = os.path.join(
        log_dir,
        f"temp_sweep_progress_resume_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
    )
    ...
    for i, T in enumerate(temperatures):
        ...
        if samples:
            ...
            print(
                f"  T={T:.0f}K: η = {eta_temp[i]:.4f} ± {eta_temp_err[i]:.4f} ({len(samples)} samples)"
            )
...
def run_disorder_with_resume(cfg, H, time_points):
    ...
    new_log = os.path.join(
        log_dir,
        f"disorder_sweep_progress_resume_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
    )
    with open(new_log, "w") as f:
