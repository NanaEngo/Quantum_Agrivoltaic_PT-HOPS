...
def run_full_fmo_simulation(cfg):
    ...
    for label in ["filtered", "broadband"]:
        ...
        if checkpoint.is_step_complete(step_name):
            ...
            logger.info(
                f"Skipping {label} simulation (already completed at {result_path})"
            )
            # Load from saved file
            df = pd.read_csv(result_path)
            results[label] = {
                "populations": df[
                    [c for c in df.columns if c.startswith("population_")]
                ].values,
                "coherences": df["coherences"].values,
                "t_axis": df["time_fs"].values,
                "n_traj": n_traj,
                "simulator": "MesoHOPS-Parallel",
                "ipr": df.get("ipr", np.zeros(len(df))).values
                if "ipr" in df.columns
                else None,
                "qfi": df.get("qfi", np.zeros(len(df))).values
                if "qfi" in df.columns
                else None,
                "entropy": df.get("entropy", np.zeros(len(df))).values
        ...
        try:
            ...
            if audit_data:
                ...
                if mae_residual >= convergence_threshold:
                    ...
                    logger.error("Convergence check failed")
                    sys.exit(1)
                print(
