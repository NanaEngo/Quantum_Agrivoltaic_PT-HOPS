...
def save_simulation_data_to_csv(
    ...
    summary_data = {
        "metric": [
            "final_population_site_1",
            "final_population_site_2",
            "final_population_site_3",
            "final_coherence",
            "final_qfi",
            "average_etr",
            "max_qfi",
        ...
        "value": [
            populations[-1, 0] if n_sites > 0 else 0,
...
def save_optimization_results_to_csv(
    optimization_params, optimization_results, filename="optimization_results"
):
    """
    ...
    # Add results
    if hasattr(optimization_results, "__iter__") and not isinstance(
...
def save_robustness_analysis_to_csv(
    robustness_data, temperatures, disorder_strengths, filename="robustness_analysis"
):
    """
    ...
    print(
        f"Saved robustness analysis to figures/{filename}_temperature_sensitivity.csv and figures/{filename}_disorder_sensitivity.csv"
    )
...
def save_spectral_data_to_csv(
    wavelengths, solar_irradiance, transmission_funcs, filename="spectral_data"
):
    """
