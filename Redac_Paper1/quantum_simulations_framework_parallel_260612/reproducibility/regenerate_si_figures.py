...
def generate_temperature_dynamics():
    ...
    ax.axvspan(
        285, 310, alpha=0.08, color="green", label="Physiological range (285--310 K)"
    )
    ax.axhline(
        0.38,
        color="gray",
...
def generate_filter_sweep():
    ...
    bars = ax1.bar(
        x, eta_vals, width=0.55, color=bar_colors, edgecolor="black", linewidth=0.5
    )
    ...
    ax1.set_title(
        "(a) Spectral Filter Comparison", loc="left", fontsize=12, fontweight="bold"
    )
    ...
    for i in range(len(x)):
        ax2.vlines(
            i, phi_broad, phi_filt[i], color=bar_colors[i], linewidth=1.2, alpha=0.5
        )
    ax2.set_xticks(x)
    ...
    ax2.set_title(
        "(b) Target Population Comparison", loc="left", fontsize=12, fontweight="bold"
    )
