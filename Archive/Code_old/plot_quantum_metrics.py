...
def plot_quantum_metrics():
    ...
    from quantum_agrivoltaics_simulations import (
    ...
    qd_sim = QuantumDynamicsSimulator(
    ...
    fig.suptitle(
        "Quantum Metrics Evolution in Agrivoltaic System",
        fontsize=16,
        fontweight="bold",
    )
    ...
    axes[0, 1].plot(
        time_points, entropy_values, "b-", label="von Neumann Entropy", linewidth=2
    )
    ...
    ax.annotate(
        f"Max QFI: {qfi_values[max_qfi_idx]:.2f}",
    ...
    ax.axhline(
        y=50, color="gray", linestyle="--", alpha=0.5, label="Expected range O(10-100)"
    )
    ax.legend()
