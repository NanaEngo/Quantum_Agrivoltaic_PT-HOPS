...
df = pd.read_csv(
    "../../simulation_data/quantum_dynamics_20260225_170638.csv",
    skipfooter=1,
    engine="python",
)
...
ax.axvline(
    x=tau_filtered, color=colors["filtered"], linestyle=":", linewidth=1, alpha=0.5
)
ax.axvline(x=300, color=colors["broadband"], linestyle=":", linewidth=1, alpha=0.5)
...
ax.plot(
    df["time_fs"],
    df["population_site_1"],
    color=colors["population"],
    linewidth=2,
    label="Site 1 (Initial)",
)

# Site 3 (reaction center proxy)
ax.plot(
    df["time_fs"],
    df["population_site_3"],
    color=colors["rc"],
    linewidth=2,
    label="Site 3 (RC)",
)
...
ax.plot(df["time_fs"], df["qfi"], color=colors["qfi"], linewidth=2, label="Filtered")
...
ax.plot(
    peak_time, peak_qfi, "o", color="red", markersize=8, label=f"Peak: {peak_qfi:.1f}"
)
