...
print(
    f"Data loaded. Time range: {time_ps[0]:.2f}–{time_ps[-1]:.2f} ps, {len(time_ps)} points"
)
...
ax.set_xlabel("Time (ps)")
ax.set_ylabel("$\\xi_{\\rm deloc}$ (IPR)")
...
for w0, S in modes:
    ...
    ax.annotate(
        f"{w0}", xy=(w0, J_v.max() * 1.15), fontsize=6, ha="center", color=C["orange"]
    )
ax.set_xlabel("Frequency (cm$^{-1}$)")
ax.set_ylabel("$J(\\omega)$ (a.u.)")
...
ax.annotate(
    f"Peak = {coherence.max():.3f}",
    xy=(time_ps[peak_idx], coherence[peak_idx]),
    xytext=(0.3, 0.8),
    fontsize=8,
    color=C["red"],
...
ax.set_xlabel("Time (ps)")
ax.set_ylabel("Normalised QFI ($F_Q / F_{Q,0}$)")
...
ax.set_title("(a) Global reactivity", fontweight="bold")
...
ax.annotate(
    "Readily biodegradable", xy=(0.5, 72), fontsize=7, color=C["grey"], ha="center"
)
ax.set_ylabel("$B_{\\rm index}$")
ax.set_title("(b) Biodegradability", fontweight="bold")
...
ax.set_yticks([0.3, 2.3])
ax.set_yticklabels(mats)
ax.set_xlabel("Energy (eV)")
ax.set_title("(c) Frontier orbitals", fontweight="bold")
plt.tight_layout()
fig.savefig(f"{OUT}/Global_Reactivity_Indices.pdf")
plt.close()
print("✓ SI: Global_Reactivity")

# SI: PAR Transmission
fig, ax = plt.subplots(figsize=(5.5, 4))
wl = np.linspace(400, 900, 500)
opv_abs = 0.7 * np.exp(-(((wl - 550) / 80) ** 2)) + 0.3 * np.exp(
    -(((wl - 620) / 60) ** 2)
)
T_cl = np.clip(
    1
    - opv_abs
    + 0.15 * np.exp(-(((wl - 750) / 20) ** 2))
    + 0.15 * np.exp(-(((wl - 820) / 20) ** 2)),
    0,
    1,
)
...
ax.axvspan(740, 760, alpha=0.1, color=C["green"])
ax.axvspan(810, 830, alpha=0.1, color=C["green"])
...
plt.close()
print("✓ SI: PAR_Transmission")

# SI: Response Functions
fig, ax = plt.subplots(figsize=(5.5, 4))
opv_r = 0.7 * np.exp(-(((wl - 550) / 80) ** 2)) + 0.4 * np.exp(
    -(((wl - 620) / 60) ** 2)
)
psu_r = (
    0.5 * np.exp(-(((wl - 680) / 30) ** 2))
    + 0.8 * np.exp(-(((wl - 750) / 25) ** 2))
    + 0.6 * np.exp(-(((wl - 820) / 20) ** 2))
)
...
for wc in [750, 820]:
    ax.axvspan(wc - 15, wc + 15, alpha=0.15, color=C["gold"])
ax.annotate(
    "Dual-band\nwindow",
    xy=(785, 0.85),
    fontsize=8,
...
ax.set_title("Spectral response: OPV vs PSU", fontweight="bold")
ax.legend(frameon=False)
plt.tight_layout()
fig.savefig(f"{OUT}/Response_Functions__OPV_vs_PSU.pdf")
plt.close()
print("✓ SI: Response_Functions")
...
im = ax.pcolormesh(
    months, lats, etr_map, cmap="RdYlGn", shading="auto", vmin=16, vmax=28
)
...
for name, lt in {
    ...
    ax.annotate(
        name, xy=(12.3, lt), fontsize=6, color="white", fontweight="bold", va="center"
    )
plt.tight_layout()
...
ax.set_xlabel("ETR Enhancement (%)")
ax.set_ylabel("Probability density")
ax.set_title(
    "Distribution over 100 disorder realisations ($\\sigma=50$ cm$^{-1}$)",
    fontweight="bold",
)
ax.legend(frameon=False)
plt.tight_layout()
