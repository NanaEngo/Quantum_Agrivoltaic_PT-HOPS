...
class ExecutionCheckpoint:
    ...
    def mark_step_complete(
        self, step_name: str, result_path: Optional[str] = None
    ) -> None:
        """
...
class StepByStepSaver:
    ...
    def save_convergence_audit(
        self, audit_data: Dict[str, Any], config_hash: str
    ) -> str:
        """
        ...
        filepath = os.path.join(
            self.results_dir, f"convergence_audit_{config_hash}_{timestamp}.csv"
        )
    ...
    def save_fmo_dynamics_intermediate(
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filepath = os.path.join(
            self.results_dir, f"fmo_dynamics_{label}_{config_hash}_{timestamp}.csv"
        )
    ...
    def save_temperature_sweep_progress(
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filepath = os.path.join(
            self.results_dir, f"temperature_sweep_{config_hash}_{timestamp}.csv"
        )
    ...
    def save_disorder_sampling_progress(
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filepath = os.path.join(
            self.results_dir, f"disorder_samples_{config_hash}_{timestamp}.csv"
        )
...
class ResumableParallelExecutor:
    ...
    def _load_completed_tasks(self) -> None:
        ...
        tasks_file = os.path.join(
            self.checkpoint.checkpoint_dir, "completed_tasks.json"
        )
