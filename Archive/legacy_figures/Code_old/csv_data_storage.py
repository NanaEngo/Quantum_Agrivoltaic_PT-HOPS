...
class CSVDataStorage:
    ...
    def save_simulation_data_to_csv(
        self,
        time_points,
        populations,
        coherences,
        ...
        pop_filename = os.path.join(
            self.output_dir, f"{filename_prefix}_populations_{timestamp}.csv"
        )
        ...
        real_filename = os.path.join(
            self.output_dir, f"{filename_prefix}_coherences_real_{timestamp}.csv"
        )
        ...
        imag_filename = os.path.join(
            self.output_dir, f"{filename_prefix}_coherences_imag_{timestamp}.csv"
        )
        imag_df.to_csv(imag_filename, index=False, float_format="%.8e")

        # Save QFI and ETR data
        metrics_df = pd.DataFrame(
            {"Time_fs": time_points, "QFI": qfi_values, "ETR": etr_time}
        )

        metrics_filename = os.path.join(
            self.output_dir, f"{filename_prefix}_quantum_metrics_{timestamp}.csv"
        )
        metrics_df.to_csv(metrics_filename, index=False, float_format="%.8e")

        print("  Saved simulation data to:")
    ...
    def save_optimization_results_to_csv(
        self, initial_params, opt_results, filename="optimization_results"
    ):
        """
    ...
    def save_spectral_data_to_csv(
        self, wavelengths, solar_spectrum, responses, filename="spectral_data"
    ):
        """
    ...
    def save_robustness_analysis_to_csv(
        self, robustness_data, filename="robustness_analysis"
    ):
        """
        ...
        if (
            "temperature_sensitivity" in robustness_data
            and "temperatures" in robustness_data
        ):
            ...
            temp_filename = os.path.join(
                self.output_dir, f"{filename}_temperature_{timestamp}.csv"
            )
            temp_df.to_csv(temp_filename, index=False, float_format="%.8e")
            print(f"  Saved temperature robustness data to: {temp_filename}")

        if (
            "disorder_sensitivity" in robustness_data
            and "disorder_strengths" in robustness_data
        ):
            ...
            disorder_filename = os.path.join(
                self.output_dir, f"{filename}_disorder_{timestamp}.csv"
            )
    ...
    def save_quantum_metrics_to_csv(
        self,
        time_points,
        entropy_values,
        purity_values,
    ...
    def save_biodegradability_analysis_to_csv(
        self, biodegrad_data, filename="biodegradability_analysis"
    ):
        """
