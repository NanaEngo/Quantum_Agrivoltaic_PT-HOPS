...
class EcoDesignAnalyzer:
    ...
    def __init__(
        self,
        target_pce: float = DEFAULT_PCE,
        target_biodegradability: float = TARGET_BIODEGRADABILITY,
        target_lc50: float = TARGET_LC50,
        ...
        self.reference_values = {
            "chemical_potential": REF_CHEMICAL_POTENTIAL,
            "chemical_hardness": REF_CHEMICAL_HARDNESS,
            "electrophilicity": REF_ELECTROPHILICITY,
            "max_fukui_nucleophilic": REF_FUKUI_NUC,
            "max_fukui_electrophilic": REF_FUKUI_ELEC,
        }
    ...
    def evaluate_material_sustainability_with_dft(
        ...
        bde: float = BDE_THRESHOLD_ECO,
        lc50: float = DEFAULT_LC50,
    ) -> Dict[str, Any]:
        """
    ...
    def calculate_biodegradability_index(
        ...
        size_factor = 100.0 * np.exp(-molecular_weight / BINDEX_SIZE_DECAY)

        # Weighted combination
        b_index = (
            BINDEX_WEIGHT_NUC * f_plus_max
            + BINDEX_WEIGHT_ELEC * f_minus_max
            + BINDEX_WEIGHT_REACT * reactivity_component
            + BINDEX_WEIGHT_SIZE * size_factor
        )
    ...
    def evaluate_material_sustainability(
        ...
        if bde < BDE_THRESHOLD_ECO:
            b_index += (BDE_THRESHOLD_ECO - bde) * 0.1
    ...
    def plot_analysis_results(
        ...
        plt.savefig(filepath, dpi=DEFAULT_DPI, bbox_inches="tight")  # JPCL 600 DPI
