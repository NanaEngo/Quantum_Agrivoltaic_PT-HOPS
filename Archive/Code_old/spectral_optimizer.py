...
class SpectralOptimizer:
    ...
    def __init__(self, agrivoltaic_model, quantum_simulator=None, n_processes=None):
        ...
        self.n_processes = (
            n_processes  # Store number of processes for parallel optimization
        )
    ...
    def calculate_synergistic_performance(self, transmission):
        ...
        quantum_synergy = np.sqrt(
            pce * etr
        )  # Geometric mean as a simple synergy measure
    ...
    def optimize_spectral_splitting(
        self, method="differential_evolution", maxiter=15, popsize=15
    ):
        """
                ...
                results.append(
                    {
                        ...
                        "spce": spce,
                        "objective": -(
                            0.4 * pce + 0.4 * etr + 0.2 * spce
                        ),  # For consistency with optimizer
                    }
                )
            except:
