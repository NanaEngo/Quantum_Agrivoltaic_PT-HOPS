...
phi_smooth = converged_value - (converged_value - phi_ft[0]) * np.exp(-(L_smooth - 4) / 0.8)
...
# Highlight L=8
ax.plot(8, phi_ft[4], '*', color=colors['accent_red'], markersize=20, 
        markeredgecolor='darkred', markeredgewidth=1.5, 
        label='Production runs ($L=8$)', zorder=10)
...
for i, (L, phi) in enumerate(zip(L_values, phi_ft)):
    if L != 8:
        ...
        if deviation > 1e-4:
            ax.annotate(f'{deviation:.2f}%', xy=(L, phi), xytext=(0, 10),
                        textcoords='offset points', ha='center', fontsize=8,
                        color=colors['dark_gray'])
        else:
            ax.annotate('<1e-6%', xy=(L, phi), xytext=(0, 10),
                        textcoords='offset points', ha='center', fontsize=8,
                        color=colors['dark_gray'])
