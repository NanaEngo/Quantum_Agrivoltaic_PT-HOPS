...
def create_fmo_hamiltonian(include_reaction_center=False):
    ...
    if include_reaction_center:
        ...
        site_energies = np.array(
            [12200, 12070, 11980, 12050, 12140, 12130, 12260, 11700]
        )  # Last is RC
    else:
...
energies_df = pd.DataFrame(
    {"Site_Index": range(len(fmo_energies)), "Energy_cm-1": fmo_energies}
)
...
plt.tight_layout()
plt.savefig(
    os.path.join(FIGURES_DIR, "FMO_Site_Energies.pdf"), bbox_inches="tight", dpi=300
)
plt.savefig(
    os.path.join(FIGURES_DIR, "FMO_Site_Energies.png"), bbox_inches="tight", dpi=300
)
plt.show()
...
class QuantumDynamicsSimulator:
    ...
    def calculate_etr(self, populations, time_points):
        ...
        if time_interval > 0:
            energy_transferred / time_interval
        else:
            pass
        ...
        etr_avg = (
            np.mean(populations[:, 1:]) if populations.shape[1] > 1 else 0.0
        )  # Average population in other sites
    ...
    def simulate_dynamics(self, initial_state=None, time_points=None, use_tensor=True):
        ...
        dt * 5308.8  # Convert fs to cm^-1 units
        ...
        for i, t in enumerate(time_points):
            ...
            try:
                bipartite_ent_values[i] = self.calculate_bipartite_entanglement(
                    current_rho
                )
            except:
            ...
            try:
                multipartite_ent_values[i] = self.calculate_multipartite_entanglement(
                    current_rho
                )
            except:
            ...
            try:
                pairwise_concurrence_values[i] = self.calculate_pairwise_concurrence(
                    current_rho
                )
            except:
                ...
                if use_tensor:
                    ...
                    for site in range(n_sites):
                        ...
                        for site2 in range(n_sites):
                            if site != site2:
                                d_rho[site, site2] *= np.exp(
                                    -self.dephasing_rate * dt_step_cm
                                )
    ...
    def calculate_multipartite_entanglement(self, rho):
        ...
        try:
            ...
            if (
                n < 4
            ):  # Need at least 4-level system for meaningful multipartite measure
                return 0.0
...
class SolarSpectrumIntegrator:
    ...
    def __init__(
        ...
        wavelength_max=2500,
        n_points=2201,
        geographic_location="subtropical",
        ...
        try:
            ...
            if os.path.exists(data_path):
                ...
                self.standard_spectrum = df.set_index("Wavelength(nm)")[
                    "GlobalHorizontal"
                ].values
    ...
    def create_synthetic_spectrum(self):
        ...
        for center_wl, width in [
            ...
            mask = (self.standard_wavelengths > center_wl - width / 2) & (
                self.standard_wavelengths < center_wl + width / 2
            )
            atmospheric_filter[mask] *= 0.3  # Reduce transmission in absorption bands

        self.standard_spectrum = (
            spectral_radiance * atmospheric_filter * 1.2e-6
        ...
        self.wavelengths = np.linspace(
            self.wavelength_min, self.wavelength_max, self.n_points
        )
    ...
    def get_solar_spectrum(
        ...
        try:
            ...
            spectrum[self.wavelengths < 400] = 0.8 * np.exp(
                -(400 - self.wavelengths[self.wavelengths < 400]) / 50
            )
            spectrum[self.wavelengths > 700] = 1.5 * np.exp(
                -(self.wavelengths[self.wavelengths > 700] - 700) / 200
            )

        # Apply seasonal variation based on latitude and day of year
        day_of_year = (month - 1) * 30 + day  # Approximate
        seasonal_modulation = 1.0 + 0.1 * self.params["seasonal_factor"] * np.cos(
        ...
        air_mass = 1.0 / np.cos(
            np.radians(
                90 - max(5, 45 - 23.4 * np.cos(2 * np.pi * day_of_year / 365.25))
            )
        )
        ...
        location_factor = 1.0 - 0.05 * (
            self.params["latitude"] / 90.0
        )  # Latitude correction
...
class EcoDesignAnalyzer:
    ...
    def calculate_fukui_functions(self, hamiltonian=None, overlap_matrix=None):
        ...
        if hamiltonian is None:
            ...
            np.fill_diagonal(
                hamiltonian, np.random.uniform(12000, 13000, n_sites)
            )  # Site energies
            for i in range(n_sites):
                for j in range(i + 1, n_sites):
                    # Add electronic couplings
                    if abs(i - j) <= 2:  # Nearest and next-nearest neighbors
                        hamiltonian[i, j] = hamiltonian[j, i] = np.random.uniform(
                            50, 300
                        )
    ...
    def calculate_global_reactivity_indices(
        self, f_plus=None, f_minus=None, f_zero=None
    ):
        """
        ...
        dual_descriptor = (
            np.max(f_plus) - np.max(f_minus)
            if len(f_plus) > 0 and len(f_minus) > 0
            else 0.0
        )
    ...
    def calculate_biodegradability_score(
        self,
        molecular_type="pm6_derivative",
        n_hydrolyzable_bonds=None,
        bond_dissociation_energy=None,
    ):
        """
    ...
    def calculate_lca_impact(
        ...
        operational_output = (
            operational_time * 365 * 24 * 0.15 * efficiency_factor
        )  # Wh/m²
        ...
        total_impact = (
            manufacturing_impact + transport_impact + installation_impact
        ) - co2_avoided

        # Calculate payback time
        energy_payback_time = (
            manufacturing_energy / (0.15 * 365 * 24 * efficiency_factor)
            if (0.15 * 365 * 24 * efficiency_factor) > 0
            else float("inf")
        )
        ...
        lca_results = {
            ...
            "carbon_footprint_gco2eq_per_kwh": (total_impact * 1000)
            / (operational_output / 1000)
            if operational_output > 0
            else float("inf"),
        }
    ...
    def perform_comprehensive_ecodesign_assessment(self, fmo_hamiltonian=None):
        ...
        avg_biodegradability = np.mean(
            [s["biodegradability_score"] for s in scores.values()]
        )
        ...
        lca_score = 1.0 / (
            1.0 + lca_results["carbon_footprint_gco2eq_per_kwh"] / 100.0
        )  # Normalize
        eco_design_score = (
            0.4 * avg_biodegradability + 0.3 * (avg_pce / 0.20) + 0.3 * lca_score
        )
        ...
        comprehensive_assessment = {
            "quantum_reactivity": {
                "fukui_functions": {
                    ...
                    "f_zero": f_zero.tolist()
                    if isinstance(f_zero, np.ndarray)
                    else f_zero,
                },
            ...
            "recommendations": self.get_ecodesign_recommendations(
                eco_design_score,
                lca_results["energy_payback_time_years"],
                avg_biodegradability,
            ),
        }
    ...
    def get_ecodesign_recommendations(self, eco_score, payback_time, biodegradability):
        ...
        if eco_score < 0.5:
            recommendations.append(
                "Significant improvements needed in overall eco-design"
            )
        elif eco_score < 0.7:
        ...
        if payback_time > 2.0:
            recommendations.append(
                f"Energy payback time is high ({payback_time:.1f} years), consider materials/processes with lower embodied energy"
            )
        else:
            recommendations.append(
                f"Good energy payback time ({payback_time:.1f} years)"
            )

        if biodegradability < 0.4:
            recommendations.append(
                "Improve biodegradability by adding hydrolyzable linkages"
            )
        ...
        recommendations.append(
            "Consider using renewable feedstocks for chemical synthesis"
        )
        recommendations.append(
            "Optimize manufacturing processes for lower environmental impact"
        )
...
def export_quantum_metrics(
    ...
    # Export coherences
    coh_df = pd.DataFrame(coherences, columns=["Coherence"], index=time_points)
    ...
    if isinstance(analysis_results, dict):
        ...
        if "quantum_reactivity" in analysis_results:
            ...
            if "global_reactivity_indices" in qreact:
                ...
                reactivity_df.to_csv(
                    os.path.join(DATA_DIR, "global_reactivity_indices.csv")
                )
...
def export_fmo_hamiltonian_data(hamiltonian, site_energies):
    ...
    energies_df = pd.DataFrame(
        {"Site_Index": range(len(site_energies)), "Energy_cm-1": site_energies}
    )
...
fmo_hamiltonian, fmo_energies = create_fmo_hamiltonian()
print(
    f"  - FMO Hamiltonian: {fmo_hamiltonian.shape[0]}x{fmo_hamiltonian.shape[1]} matrix"
)
print(
    f"  - Site energies range: {fmo_energies.min():.0f} to {fmo_energies.max():.0f} cm⁻¹"
)
...
print("\n2. Quantum Dynamics Simulation:")
quantum_sim = QuantumDynamicsSimulator(
    fmo_hamiltonian, temperature=295, dephasing_rate=20
)
...
print(
    f"  - Biodegradability: {np.mean([s['biodegradability_score'] for s in analysis['biodegradability_analysis'].values()]):.3f}"
)
print(
    f"  - Carbon footprint: {analysis['lca_assessment']['carbon_footprint_gco2eq_per_kwh']:.1f} gCO2eq/kWh"
)
...
summary_data = {
    ...
    "par_efficiency": par_eff,
    "lca_carbon_footprint": analysis["lca_assessment"][
        "carbon_footprint_gco2eq_per_kwh"
    ],
}
...
print(
    f"Key results: Eco-design score = {analysis['eco_design_score']:.3f}, PAR efficiency = {par_eff:.3f}"
)
