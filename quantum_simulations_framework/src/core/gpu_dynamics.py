...
class GPUQuantumDynamics:
    ...
    def __init__(
        ...
        if self.use_gpu:
            ...
            logger.info(
                f"GPU dynamics initialized: {self.n_sites} sites, T={temperature}K"
            )
        else:
            logger.info(
                f"CPU dynamics initialized: {self.n_sites} sites, T={temperature}K"
            )
    ...
    def simulate_batch_trajectories(
        self, initial_states: np.ndarray, time_points: np.ndarray, method: str = "rk4"
    ) -> np.ndarray:
        """
    ...
    def _simulate_batch_cpu(
        ...
        trajectories = np.zeros(
            (batch_size, n_times, self.n_sites, self.n_sites), dtype=np.complex128
        )
...
def gpu_ensemble_average(
    trajectories: np.ndarray, axis: int = 0, backend: str = "auto"
) -> np.ndarray:
    """
...
def gpu_batch_coherence(
    density_matrices: np.ndarray, backend: str = "auto"
) -> np.ndarray:
    """
