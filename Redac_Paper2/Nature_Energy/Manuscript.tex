...
\begin{table}[htbp]
\centering
\caption{\textbf{NPoM mode volume sensitivity of the forward transfer yield.}
Baseline (no NPoM): $\Phi_{\mathrm{FT}} = 0.98$. All NPoM runs with $n=2$ trajectories at $L=8$, $K=2$, $\Delta t = \SI{0.2}{\fs}$, $t_{\mathrm{max}} = \SI{1}{\ps}$.}
\label{tab:npom_volume_scan}
\begin{tabular}{c c c}
\toprule
$V_{\mathrm{mode}}$ (\si{\nm\cubed}) & $\Phi_{\mathrm{FT}}$ & $\Delta$ vs.\ baseline \\
\midrule
0.2  & 0.0505 & $-$94.8\% \\
0.4  & 0.0605 & $-$93.8\% \\
0.6  & 0.0727 & $-$92.6\% \\
...
The SERS enhancement factor ($\sim 10^2$ at $V_{\mathrm{mode}} = \SI{0.8}{\nm\cubed}$) must therefore be weighed against the \SI{92}{\percent} reduction in photochemical yield when designing the integrated quantum agrivoltaic device.
For applications where SERS-based diagnostics are essential, $V_{\mathrm{mode}} \approx \SI{1.2}{\nm\cubed}$ offers the optimal balance: maximal transport yield ($\Phi_{\mathrm{FT}} = 0.0804$) with a SERS enhancement of $\sim 10^2$ (\Cref{fig:npq_switch}c).
