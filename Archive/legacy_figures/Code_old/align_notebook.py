...
for cell in nb["cells"]:
    if cell["cell_type"] == "code" and any(
        "def create_fmo_hamiltonian" in line for line in cell["source"]
    ):
        new_source = []
...
for cell in nb["cells"]:
    if cell["cell_type"] == "code" and any(
        "class QuantumDynamicsSimulator" in line for line in cell["source"]
    ):
        new_source = []
        in_docstring = False
        ...
        for line in cell["source"]:
            ...
            if in_docstring and "Quantum dynamics simulator" in line:
                ...
                new_source.append("    \n")
                new_source.append(
                    "    Scientific Note: This implementation uses a computationally optimized Markovian\n"
                )
                new_source.append(
                    "    Lindblad approach to approximate the results of the full non-Markovian\n"
                )
                new_source.append(
                    "    PT-HOPS+LTC method described in the main manuscript.\n"
                )
            else:
                new_source.append(line)
