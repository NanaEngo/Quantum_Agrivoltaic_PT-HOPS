        ...
        if not os.path.exists(config_path):
            ...
            if not os.path.exists(config_path):
                raise FileNotFoundError(
                    f"Config file not found: {custom_path} (checked CWD and {_FRAMEWORK_DIR})"
                )
    else:
...
def check_environment() -> bool:
    ...
    try:
        ...
        print(
            "  ❌ MesoHOPS NOT found. Run: mamba run -n MesoHOP-sim python reproducibility/main.py"
        )
...
def _build_initial_state_for_label(
    H: np.ndarray,
    label: str,
...
def run_full_fmo_simulation(cfg: Dict[str, Any]) -> Tuple[Dict[str, Any], np.ndarray]:
    """
    Run the full FMO ensemble simulation (filtered + broadband excitation).
    ...
    for label in ["filtered", "broadband"]:
        ...
        sim = HopsSimulator(
            ...
            vibronic_in_hierarchy=dyn.get("vibronic_in_hierarchy", True),
    ...
    # ── Write a CSV immediately after each label is available ────────────────
    for _label in ["filtered", "broadband"]:
        _r = results[_label]
        _n = _r["populations"].shape[0]
        try:
            _partial_path = storage.save_quantum_dynamics_results(
                _r["t_axis"][:_n],
                _r["populations"],
                _r["coherences"],
                {},
                filename_prefix=f"fmo_dynamics_{_label}",
                config_dict=cfg,
            )
            print(f"  💾 FMO {_label} dynamics saved → {_partial_path}")
            logger.info(f"FMO {_label} dynamics saved to {_partial_path}")
        except Exception as _e:
            logger.warning(f"Per-label CSV save failed for {_label}: {_e}")

    # ── Combined CSV with both labels for downstream figure generation ───────
    metrics = {
...
def _run_trajectory_worker(
    T, label, seed, lock, log_path, bath, dyn, pulse_cfg, filter_cfg, H, time_points
):
    from src.core.hops_simulator import HopsSimulator
    ...
    try:
        ...
        # Transfer yield: long-time population of target site (Site 3 = RC exit)
        # See Rev 3 Pt 1: multi-state system requires target population, not 1 - survival
        phi = data["populations"][-1, FMO_TARGET_SITE]
...
def _run_single_disorder(
    ...
    for label in ["filtered", "broadband"]:
        ...
        try:
            ...
            print(f"🚨 OOM: disorder seed={seed}, {label}")
            eta_vals[label] = None
        except Exception:
            eta_vals[label] = None
...
def _run_temperature_sweep(
    cfg: Dict[str, Any], H: np.ndarray, time_points: np.ndarray
    ...
    sweep_log_path = os.path.join(
        _LOG_DIR, f"temp_sweep_progress_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    )
    ...
    estimate_memory_per_traj(L, K, n_modes, time_max_fs=time_max)
    batch_size = n_jobs
    n_batches = (len(temperatures) * n_traj_sweep * 2 + batch_size - 1) // batch_size
    logger.info(f"Batch-wise execution: {n_batches} batch(es) × {batch_size} traj")

    results_flat = []
    for batch_idx in range(n_batches):
        start = batch_idx * batch_size
        end = min(start + batch_size, len(temperatures) * n_traj_sweep * 2)
        batch_tasks = []
        idx = start
        for T in temperatures:
            for i in range(n_traj_sweep):
                for label, base_seed in [("filtered", 1000), ("broadband", 2000)]:
                    if idx >= start and idx < end:
                        batch_tasks.append((T, label, base_seed + i))
                    idx += 1
                    if idx >= end:
                        break
                if idx >= end:
...
def _build_disorder_samples(cfg, H, time_points, n_samples=100, rng_seed=42):
    ...
    pulse_cfg = cfg.get("pulse", {})
    filter_cfg = cfg.get("spectral_filter", None)
    sigma_disorder = cfg.get("simulation", {}).get(
    ...
    disorder_log_path = os.path.join(
        _LOG_DIR,
        f"disorder_sweep_progress_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
    )
...
def _generate_spectral_figure(cfg, output_dir):
    ...
    fmo_abs = 0.6 * np.exp(-((wavelengths_nm - 750) ** 2) / (2 * 15**2)) + 1.0 * np.exp(
        -((wavelengths_nm - 805) ** 2) / (2 * 20**2)
    )

    # ── Solar irradiance (AM1.5G approximation) ─────────────────────────────
    solar = 0.8 * np.exp(-((wavelengths_nm - 550) ** 2) / (2 * 150**2)) + 0.4 * np.exp(
    ...
    vib_damp_cm = (
        np.array(vib_damp_raw)
        if isinstance(vib_damp_raw, list)
...
def generate_figures(cfg, sim_results, time_points, skip_temp_sweep=False):
    ...
    jpcl_dir = os.path.abspath(
        os.path.join(_SCRIPT_DIR, "..", "..", "Theory_Journals_main", "JPCL")
    )
...
def main():
    ...
    if not getattr(args, "skip_audit", False):
        ...
        if mae_residual >= convergence_threshold:
            ...
            logger.error(
                f"Convergence check failed: Residual MAE={mae_residual:.2e} >= {convergence_threshold:.2e}"
            )
            sys.exit(1)
        print(
            f"  ✅ Convergence confirmed: Residual MAE={mae_residual:.2e} < {convergence_threshold:.2e}"
        )
    else:
    ...
    print("  Figures → Theory_Journals/JPCL/")
