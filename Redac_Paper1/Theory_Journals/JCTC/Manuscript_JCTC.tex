...
\title{Scalable simulation of non-Markovian energy transfer via Process Tensor Hierarchy of Pure States with Spectrally Bundled Dissipators}
...
In this work, we integrate PT-HOPS with Spectrally Bundled Dissipators (SBD)\cite{Citty2024a} and present a comprehensive benchmarking of the combined methodology. We establish numerical exactness via a 12-test validation suite, characterize the computational scaling with hierarchy depth and Padé expansion order, and demonstrate the algorithm on a physically relevant application: excitonic dynamics in the FMO complex under spectrally engineered incident radiation. We compare our approach with HEOM\cite{Ishizaki2009a} and the recently developed TEMPO method\cite{Strathearn2018} to contextualize its performance.
