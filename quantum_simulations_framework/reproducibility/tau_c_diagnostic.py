...
def fit_mono(t: np.ndarray, C: np.ndarray,
             t_range: tuple[float, float] = (0, 1000)
             ) -> dict:
    """
    ...
    try:
        popt, pcov = curve_fit(
            mono_exp, t_fit, C_fit, p0=p0,
            bounds=([0, 1, 0], [10, 5000, 10]),
            maxfev=20000
        )
        ...
        return {"tau_c": np.nan, "error": np.nan, "rmse": np.nan,
                "popt": None, "note": str(e)}
    return {
        "tau_c": tau_c,
        "error": dtau,
        "rmse":  rmse,
...
def fit_biexp(t: np.ndarray, C: np.ndarray,
              t_range: tuple[float, float] = (0, 1000)
              ) -> dict:
    """
    ...
    try:
        ...
        return {"tau_fast": np.nan, "tau_slow": np.nan,
                "error_fast": np.nan, "error_slow": np.nan,
                "rmse": np.nan, "popt": None, "note": str(e)}
    return {
        "tau_fast":   popt[1],
        "tau_slow":   popt[3],
        "A_fast":     popt[0],
...
def hilbert_tau(t: np.ndarray, C: np.ndarray) -> dict:
    ...
    tau_c = t[idx[0]]
    return {"tau_c": tau_c, "Cinf": Cinf, "C_peak": C_peak,
            "threshold": threshold, "envelope": envelope}
...
print(f"                  C0 = {filt_mono_full['C0']:.4f}, "
      f"C_∞ = {filt_mono_full['Cinf']:.4f}")
...
print(f"\n[Bi-exp 0–1000 fs]")
print(f"  τ_fast = {filt_biexp['tau_fast']:.0f} ± {filt_biexp['error_fast']:.0f} fs  "
      f"(A_fast = {filt_biexp['A_fast']:.4f})")
print(f"  τ_slow = {filt_biexp['tau_slow']:.0f} ± {filt_biexp['error_slow']:.0f} fs  "
...
print(f"[Bi-exp 0–1000 fs] τ_fast = {broad_biexp['tau_fast']:.0f} fs, "
...
if h1_confirmed:
    ...
    print(f"   Ceci explique la valeur 420(35) fs du manuscrit.")
else:
    print(f"\n❓ H1 non confirmée : fit mono donne τ_c = {tau_mono_filt:.0f} fs")
    print(f"   (attendu : ~420 fs d'après le manuscrit)")

if biexp_better:
    print(f"\n✅ MODÈLE BIEXPONENTIEL supérieur au modèle monoexponentiel :")
    print(f"   RMSE mono = {rmse_mono:.5f}  vs  RMSE bi-exp = {rmse_biexp:.5f}")
    print(f"   → Physiquement cohérent avec sélection spectrale de modes vibroniques")
else:
    print(f"\n⚠️  RMSE mono ({rmse_mono:.5f}) comparable à bi-exp ({rmse_biexp:.5f})")
    print(f"   → Signal peu oscillant ou bruit dominant")

print(f"\n📊 RECOMMANDATION MANUSCRIT :")
print(f"   Remplacer τ_c = 420(35) fs par :")
...
print(f"   τ_c broadband = {broad_mono['tau_c']:.0f} ± {broad_mono['error']:.0f} fs (mono-exp, valide)")
...
def style_ax(ax, title=""):
    ...
    ax.legend(facecolor="#1a1d27", edgecolor=SPINE_COLOR,
              labelcolor=TEXT_COLOR, fontsize=8, framealpha=0.9)


# — Panel (a) : Signal filtré + fits mono vs bi-exp
ax1 = fig.add_subplot(gs[0, 0])
ax1.plot(t_filt, C_filt, color=COLORS["filtered"], lw=1.5,
         label="C_{l₁}^{filt}(t) — données L=8", zorder=3)

# Fit mono [0-1000]
t_plot = np.linspace(0, 1000, 2000)
if filt_mono_full["popt"] is not None:
    ax1.plot(t_plot, mono_exp(t_plot, *filt_mono_full["popt"]),
             color=COLORS["mono"], lw=2, ls="--",
             label=f"Mono-exp: τ={filt_mono_full['tau_c']:.0f} fs (H1 = manuscrit)")

# Fit bi-exp
if filt_biexp["popt"] is not None:
    ax1.plot(t_plot, bi_exp(t_plot, *filt_biexp["popt"]),
             color=COLORS["biexp_fast"], lw=2, ls="-.",
             label=(f"Bi-exp: τ_f={filt_biexp['tau_fast']:.0f} fs, "
                    f"τ_s={filt_biexp['tau_slow']:.0f} fs"))
...
ax2.plot(t_broad, C_broad, color=COLORS["broadband"], lw=1.5,
         label="C_{l₁}^{broad}(t) — données L=8", zorder=3)
if broad_mono["popt"] is not None:
    ax2.plot(t_plot, mono_exp(t_plot, *broad_mono["popt"]),
             color=COLORS["mono"], lw=2, ls="--",
             label=f"Mono-exp: τ={broad_mono['tau_c']:.0f} fs")
if broad_biexp["popt"] is not None:
    ax2.plot(t_plot, bi_exp(t_plot, *broad_biexp["popt"]),
             color=COLORS["biexp_fast"], lw=2, ls="-.",
...
ax3.plot(t_filt, C_filt - Cinf_filt, color=COLORS["filtered"], lw=1,
         alpha=0.6, label="Signal oscillant (− C_∞)")
if envelope_filt is not None:
    ax3.plot(t_filt, envelope_filt, color=COLORS["hilbert"], lw=2,
             label="Enveloppe Hilbert")
    threshold = filt_hilbert.get("threshold", 0)
    ax3.axhline(y=threshold, color=COLORS["manuscript"], lw=1.5, ls="--",
                label=f"Seuil 1/e = {threshold:.3f}")
    tau_h = filt_hilbert["tau_c"]
    if np.isfinite(tau_h):
        ax3.axvline(x=tau_h, color=COLORS["hilbert"], lw=1.5, ls=":",
...
summary_data = [
    ...
    ["Hilbert 1/e",
     f"{filt_hilbert['tau_c']:.0f}",
     f"{broad_hilbert['tau_c']:.0f}"],
    ["────────────────────", "──────────────", "──────────────"],
    ["Bi-exp τ_fast",
    ...
    ["────────────────────", "──────────────", "──────────────"],
    ["RMSE mono-exp",
     f"{filt_mono_full['rmse']:.5f}",
     f"{broad_mono['rmse']:.5f}"],
    ["RMSE bi-exp",
     f"{filt_biexp['rmse']:.5f}",
     f"{broad_biexp['rmse']:.5f}"],
]
...
with open(report_path, "w") as f:
    ...
    f.write("=" * 60 + "\n")
    f.write(f"Mono-exp [0-1000 fs]: tau_c = {filt_mono_full['tau_c']:.0f} +/- "
            f"{filt_mono_full['error']:.0f} fs  (RMSE = {filt_mono_full['rmse']:.5f})\n")
    f.write(f"Bi-exp tau_fast:      {filt_biexp['tau_fast']:.0f} +/- "
            f"{filt_biexp['error_fast']:.0f} fs  (A_fast = {filt_biexp['A_fast']:.4f})\n")
    f.write(f"Bi-exp tau_slow:      {filt_biexp['tau_slow']:.0f} +/- "
    ...
    f.write(f"Manuscript value:     420 +/- 35 fs\n\n")

    f.write("=" * 60 + "\n")
    f.write("BROADBAND SIGNAL\n")
    f.write("=" * 60 + "\n")
    f.write(f"Mono-exp [0-1000 fs]: tau_c = {broad_mono['tau_c']:.0f} +/- "
            f"{broad_mono['error']:.0f} fs  (RMSE = {broad_mono['rmse']:.5f})\n")
    f.write(f"Bi-exp tau_fast:      {broad_biexp['tau_fast']:.0f} +/- "
            f"{broad_biexp['error_fast']:.0f} fs\n")
    f.write(f"Bi-exp tau_slow:      {broad_biexp['tau_slow']:.0f} +/- "
            f"{broad_biexp['error_slow']:.0f} fs\n")
    f.write(f"Hilbert 1/e:          {broad_hilbert['tau_c']:.0f} fs\n")
    f.write(f"Manuscript value:     280 +/- 25 fs\n\n")
