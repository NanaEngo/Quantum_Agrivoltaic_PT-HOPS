...
class GPUQuantumDynamics:
    ...
    def __init__(self, hamiltonian: np.ndarray, temperature: float = 295.0,
                 use_gpu: bool = True):
        """
...
m))
    ...
    return np.array(coherences_gpu)
urn np.array(avg_gpu)


@jit
def gpu_coherence_l1_norm(rho):
    """Compute l1-norm coherence on GPU (JIT-compiled)."""
    # Zero out diagonal
    n = rho.shape[0]
    mask = 1.0 - jnp.eye(n)
    off_diag = rho * mask
    return jnp.sum(jnp.abs(off_diag))


def gpu_batch_coherence(density_matrices: np.ndarray) -> np.ndarray:
    """
    Compute coherence for batch of density matrices on GPU.
    ...
    # Vectorize over batch and time dimensions
    batched_coherence = vmap(vmap(gpu_coherence_l1_norm))
    coherences_gpu = batched_coherence(rho_gpu)
