...
def test_quantum_simulation():
    ...
    print(
        f"   Time points: {len(time_points)} points from {time_points[0]} to {time_points[-1]} fs"
    )
    ...
    print("   Initial state: excitation on site 0")
    ...
    try:
        ...
        if "qfi" in results or "entropy" in results or "purity" in results:
            ...
            if "qfi" in results:
                axes[0].plot(
                    results["t_axis"], results["qfi"], linewidth=2, color="#3498db"
                )
            ...
            if "entropy" in results:
                axes[1].plot(
                    results["t_axis"], results["entropy"], linewidth=2, color="#2ecc71"
                )
            ...
            if "purity" in results:
                axes[2].plot(
                    results["t_axis"], results["purity"], linewidth=2, color="#9b59b6"
                )
