...
transmission = np.exp(-((wavelengths - 750) ** 2) / 2 / 25**2) + np.exp(
    -((wavelengths - 820) ** 2) / 2 / 25**2
)

# Plot transmission spectrum on filter
trans_x = 0.25 + 0.5 * (transmission / transmission.max())
ax_center.plot(
    trans_x, 0.15 + 0.7 * (wavelengths - 400) / 500, color="white", linewidth=2
)

# Mark transmission peaks
ax_center.plot(
    [0.75, 0.75], [0.15, 0.82], color="yellow", linestyle="--", linewidth=1.5, alpha=0.8
)
ax_center.plot(
    [0.75, 0.75], [0.15, 0.88], color="yellow", linestyle="--", linewidth=1.5, alpha=0.8
)
...
for y in [0.3, 0.5, 0.7]:
    ax_center.arrow(
        0.15, y, 0.08, 0, color="white", linewidth=2, head_width=0.03, head_length=0.02
    )
    ax_center.arrow(
        0.82,
        y,
...
for pos in fmo_positions[:3]:
    for r in [0.12, 0.18, 0.24]:
        circle = Circle(
            pos, r, fill=False, color=colors["coherence"], linewidth=0.5, alpha=0.3
        )
        ax_right.add_patch(circle)
...
fig.text(
    0.5,
    0.98,
    "Quantum-Coherent Spectral Engineering",
    ha="center",
    va="top",
    fontsize=16,
    fontweight="bold",
...
plt.savefig(
    output_file, dpi=100, bbox_inches="tight", facecolor="white", edgecolor="none"
)
