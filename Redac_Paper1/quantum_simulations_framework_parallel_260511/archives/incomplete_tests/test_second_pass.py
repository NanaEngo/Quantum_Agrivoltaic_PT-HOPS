...
from models.quantum_dynamics_simulator import spectral_density_drude_lorentz
...
class TestSimpleSimulator:
    ...
    def test_initial_state_wrong_length_raises(self, sim7, short_t):
        bad_psi = np.array([1.0, 0.0, 0.0])  # mismatched length
    ...
    def test_populations_shape(self, sim7, short_t, psi1):
        ...
        assert r["populations"].shape == (len(short_t), psi1.shape[0])

    def test_populations_non_negative(self, sim7, short_t, psi1):
        r = sim7.simulate_dynamics(short_t, psi1)
        assert np.all(r["populations"] >= -1e-6) # Stochastic floor
    ...
    def test_initial_population_site3(self, H7, short_t):
        ...
        psi = np.zeros(H7.shape[0], dtype=complex)
        psi[min(2, H7.shape[0]-1)] = 1.0
        r = sim.simulate_dynamics(short_t, psi)
        assert r["populations"][0, min(2, H7.shape[0]-1)] > 0.99
    ...
    def test_qfi_correct_formula(self, H7, short_t):
        ...
        assert abs(r["qfi"][0] - expected_qfi) < MAE_THRESHOLD
    ...
    def test_coherence_zero_at_t0_site_basis(self, H7, short_t):
        ...
        assert r["coherences"][0] < POPS_REAL_THRESHOLD
    ...
    def test_default_time_points(self, sim7, psi1):
        ...
        assert r["populations"].shape[0] > 0
...
class TestSBD:
    ...
    def test_all_same_frequency(self):
        ...
        sbd = StochasticallyBundledDissipator(n_bundles=3)  # n_sites/n_modes not in constructor
        modes = [(200.0, 0.1)] * 6
        bundles = sbd.discretize_spectral_density(modes)
        assert len(bundles) >= 1
...
class TestOptimizeLogic:
    ...
    def test_phi_ft_is_float(self):
        ...
        n_sites = 7
        pops = np.random.rand(100, n_sites)
...
class TestRegressions:
    ...
    def test_spectral_density_elementwise_sign(self):
        ...
        from core.constants import DEFAULT_REORGANIZATION_ENERGY, DEFAULT_DRUDE_CUTOFF, DEFAULT_TEMPERATURE
    ...
    def test_csv_no_slash_in_filename(self, tmp_path, H7):
    ...
    def test_figure_generator_length_guard(self, tmp_path, H7):
        ...
        n_sites = H7.shape[0]
        pops = np.random.rand(45, n_sites)
    ...
    def test_simple_sim_qfi_not_var_diag(self, H7, short_t):
        ...
        H_shifted = H7 - np.mean(np.diag(H7).real) * np.eye(n_sites)
        exp_H = np.real(np.vdot(psi, H_shifted @ psi))
        exp_H2 = np.real(np.vdot(psi, H_shifted @ H_shifted @ psi))
