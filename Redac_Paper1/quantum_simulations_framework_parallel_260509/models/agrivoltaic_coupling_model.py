...
class AgrivoltaicCouplingModel:
    ...
    def __init__(
        ...
        fmo_ham_eV = fmo_hamiltonian / EV_TO_CM
        ...
        if solar_spectrum is None:
            # Create standard AM1.5G spectrum in the range SOLAR_LAMBDA_MIN to SOLAR_LAMBDA_MAX
            lambda_range = np.linspace(SOLAR_LAMBDA_MIN, SOLAR_LAMBDA_MAX, 801)  # nm
            ...
            for i, lam in enumerate(lambda_range):
                if SOLAR_LAMBDA_MIN <= lam <= 400:  # UV-Violet
                    solar_irradiance[i] = 0.5 + 1.2 * (lam - SOLAR_LAMBDA_MIN) / 100
    ...
    def _calculate_opv_response(self):
        ...
        for i, lam in enumerate(self.lambda_range):
            if lam > lambda_cutoff:
                ...
                efficiency_factor = (
                    1.0 - (lam - lambda_cutoff) / OPV_RESPONSE_DECAY_WIDTH
                ...
                R_opv[i] = self.opv_absorption_coeff * ETR_MIN * (lam / lambda_cutoff)
    ...
    def _calculate_psu_response(self):
        ...
        for i, lam in enumerate(self.lambda_range):
            if PSU_BLUE_MIN <= lam <= PSU_BLUE_MAX:  # Blue region
                # Chlorophyll absorption peak in blue
                R_psu[i] = 0.85 + 0.15 * np.sin(np.pi * (lam - PSU_BLUE_MIN) / 100)
            elif PSU_RED_MIN <= lam <= PSU_RED_MAX:  # Red region
                # Chlorophyll absorption peak in red
                R_psu[i] = 0.88 + 0.12 * np.cos(np.pi * (lam - 650) / 50)
            elif PSU_GREEN_MIN < lam < PSU_GREEN_MAX:  # Green valley
                # Lower efficiency in green region
                R_psu[i] = 0.25 + 0.05 * np.sin(np.pi * (lam - PSU_GREEN_MIN) / 100)
            elif lam < PSU_BLUE_MIN:  # UV region
    ...
    def calculate_opv_current(self, transmission_func):
        ...
        current_mA_per_cm2 = current * DEFAULT_CONVERSION_FACTOR_OPV  # Reduced conversion factor for realistic current
    ...
    def calculate_opv_efficiency(self, transmission_func, voltage=0.85):
        ...
        pce = max(0.0, min(PCE_MAX_LIMIT, pce))  # Cap at 25% maximum
    ...
    def calculate_psu_efficiency(self, transmission_func):
        ...
        if par_absorbed > 0:
            ...
            etr = ETR_BASE + ETR_SCALE * (
                par_absorbed / (par_absorbed + ETR_SAT_PAR)
            )  # Asymptotic approach to max efficiency
        else:
            etr = ETR_MIN  # Very low efficiency with no light

        # Ensure realistic range (85-95% for good conditions)
        etr = max(ETR_MIN, min(ETR_MAX, etr))
    ...
    def _construct_agrivoltaic_hamiltonian(self, spectral_coupling_strength=AGRI_SPECTRAL_COUPLING_STRENGTH):
        """
    ...
    def calculate_psu_absorption_from_hamiltonian(self, omega):
        ...
        broadening = PSU_BROADENING_EV  # eV
    ...
    def simulate_energy_transfer(self, time_points, initial_state=None):
        ...
        """
        hbar_eV_fs = HBAR_EV_FS  # hbar in eV*fs
    ...
    def optimize_spectral_splitting(self, n_filters=3, maxiter=50):
        ...
        for _i in range(n_filters):
            ...
            bounds.append((SOLAR_LAMBDA_MIN, SOLAR_LAMBDA_MAX))
            # Width (5-100 nm)
...
if __name__ == "__main__":
    ...
    # Initialize model
    model = AgrivoltaicCouplingModel(fmo_ham_eV, temperature=DEFAULT_TEMPERATURE)
