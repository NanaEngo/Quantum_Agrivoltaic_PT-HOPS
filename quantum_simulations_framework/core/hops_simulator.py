    ...
    **kwargs : Any
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Parameters
        ----------
        strict_hermiticity : bool, optional
            If True (default), raises ValueError if the Hamiltonian is not Hermitian.
            Set to False to allow non-Hermitian Hamiltonians (e.g. with imaginary
            trapping terms on the diagonal representing reaction center decay).
        """
        ...
        # (unless strict_hermiticity=False, e.g. for trapping Hamiltonians).
        ...
        if strict_hermiticity and H.ndim == 2 and H.shape[0] == H.shape[1]:
            ...
            if max_asymmetry > 1e-8:
                raise ValueError(
                    ...
                    "Set strict_hermiticity=False to allow non-Hermitian Hamiltonians "
                    "(e.g. with imaginary trapping terms)."
                )
    ...
    def _get_memory_estimate(self, n_hierarchy_modes: int = 189) -> float:
        """Estimate memory per trajectory, delegating to MemoryAwareJobScheduler."""
        try:
                ...
                for _fname in ("bcf_convert_sdl_to_exp", "bcf_convert_dl_ud_to_exp"):
                    try:
                        ...
                        _mod = importlib.import_module(
                            "mesohops.util.bath_corr_functions"
                        )
                ...
                for freq, hr, damp in zip(vib_freqs, vib_hr, vib_damping, strict=False):
                    ...
                    if _ud_bcf is not None:
                        try:
                            ...
                            ud_pairs = [
                                [ud_modes[i], ud_modes[i + 1]]
                                for i in range(0, len(ud_modes), 2)
                            ]
    ...
    def _init_fallback(self, **kwargs: Any) -> None:
        ...
        if QuantumDynamicsSimulator is not None:
            try:
                ...
                logger.warning(
                    f"Failed to initialize QuantumDynamicsSimulator fallback: {e}"
                )
        ...
        if SimpleQuantumDynamicsSimulator is not None:
            try:
                ...
                logger.error(
                    f"Failed to initialize SimpleQuantumDynamicsSimulator fallback: {e}"
                )
    ...
    def simulate_dynamics(
        ...
        **kwargs : dict
        ...
        if (
            self.use_mesohops
            and hasattr(self, "system_param")
            and self.system_param is not None
        ):
            try:
        ...
        if self.fallback_sim is not None:
            logger.warning(
                "Using fallback simulator. Results will not show L-dependence."
            )
    ...
    def _simulate_with_mesohops(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            t_max = (
                float(np.max(time_points)) if len(time_points) > 0 else DEFAULT_MAX_TIME
            )
            ...
            if self.use_sbd and SBD_HopsTrajectory is not None:
                ...
                logger.info(
                    "Engaging SBD_HopsTrajectory for compressed environmental simulation."
                )
            ...
            if MemoryAwareJobScheduler is not None:
                scheduler = MemoryAwareJobScheduler(
                    max_hierarchy, k_matsubara, n_traj, time_max_fs=t_max
                )
                # Read actual mode count from system_param (gw_sysbath is out of scope here)
                n_hierarchy_modes = (
                    len(self.system_param.get("GW_SYSBATH", []))
                    if self.system_param
                    else 189
                )
                sched_info = scheduler.validate_and_adapt(n_hierarchy_modes)
                n_jobs = sched_info["n_jobs"]
                batch_size = sched_info["batch_size"]
                n_batches = sched_info["n_batches"]

                logger.info(
                    f"Memory Plan: {n_batches} batch(es) de {batch_size} traj | "
                ...
                # Safe fallback: sequential execution to prevent OOM
                n_jobs = 1
                batch_size = n_traj
                n_batches = 1
                logger.warning(
                    "MemoryAwareJobScheduler not available, running sequentially (n_jobs=1)"
                )
            ...
            for batch_idx in range(n_batches):
                ...
                logger.info(
                    f"Batch {batch_idx + 1}/{n_batches}: seeds {start_seed}-{end_seed - 1}"
                )
                ...
                if HAS_JOBLIB and n_jobs > 1:
                    ...
                    if HAS_TQDM and kwargs.get("show_progress", True):
                        ...
                        iterable = tqdm(
                            batch_seeds, desc=desc, unit="traj", leave=False
                        )
                    ...
                    try:
                        ...
                        logger.critical(
                            f"Batch {batch_idx + 1} OOM: "
                            f"~{mem_limit:.1f} GB/traj, retrying with n_jobs={max(1, n_jobs // 2)}"
                        )
                        gc.collect()
            ...
            for i in range(n_times):
                ...
                coherences[i] = float(
                    np.sum(np.abs(rho)) - np.sum(np.abs(np.diag(rho)))
                )
