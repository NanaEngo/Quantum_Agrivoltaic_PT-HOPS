...
class TestJPCLValidationSuite:
    ...
    def test_si_1_hierarchy_convergence(self, fmo_hamiltonian, t_audit):
        ...
        sim_prev = HopsSimulator(
            fmo_hamiltonian, max_hierarchy=L_prev, k_matsubara=K_val
        )
        sim_prod = HopsSimulator(
            fmo_hamiltonian, max_hierarchy=L_prod, k_matsubara=K_val
        )

        psi0 = np.eye(fmo_hamiltonian.shape[0])[0]
        res_prev = sim_prev.simulate_dynamics(
            t_audit, initial_state=psi0, n_traj=n_traj, strict_mode=True
        )
        res_prod = sim_prod.simulate_dynamics(
            t_audit, initial_state=psi0, n_traj=n_traj, strict_mode=True
        )
    ...
    def test_si_2_matsubara_convergence(self, fmo_hamiltonian, t_audit):
        ...
        sim_k_prev = HopsSimulator(
            fmo_hamiltonian, max_hierarchy=L_val, k_matsubara=K_prev
        )
        sim_k_prod = HopsSimulator(
            fmo_hamiltonian, max_hierarchy=L_val, k_matsubara=K_prod
        )

        psi0 = np.eye(fmo_hamiltonian.shape[0])[0]
        res_k_prev = sim_k_prev.simulate_dynamics(
            t_audit, initial_state=psi0, n_traj=n_traj, strict_mode=True
        )
        res_k_prod = sim_k_prod.simulate_dynamics(
            t_audit, initial_state=psi0, n_traj=n_traj, strict_mode=True
        )

        mae = np.mean(np.abs(res_k_prod["populations"] - res_k_prev["populations"]))
        logger.info(f"Test 2 (K): K={K_prod} vs K={K_prev} MAE = {mae:.2e}")
        assert mae < MAE_THRESHOLD_LOOSE, (
            f"Matsubara truncation K={K_prod} not converged"
        )
    ...
    def test_si_3_timestep_convergence(self, fmo_hamiltonian):
        ...
        psi0 = np.eye(fmo_hamiltonian.shape[0])[0]
        res1 = sim1.simulate_dynamics(
            np.arange(0, t_max, dt_fine),
            initial_state=psi0,
            n_traj=n_traj,
            strict_mode=True,
        )
        res2 = sim1.simulate_dynamics(
            np.arange(0, t_max, dt_prod),
            initial_state=psi0,
            n_traj=n_traj,
            strict_mode=True,
        )
        ...
        logger.info(
            f"Test 3 (dt): dt={dt_prod} vs dt={dt_fine} MAE@t={t_max} = {mae:.2e}"
        )
    ...
    def test_si_4_pulse_stability(self, fmo_hamiltonian):
        ...
        sim = HopsSimulator(
            fmo_hamiltonian, max_hierarchy=max(1, L_val - 2), k_matsubara=K_val
        )

        psi0 = np.eye(fmo_hamiltonian.shape[0])[0]
        res_delta = sim.simulate_dynamics(
            t, initial_state=psi0, n_traj=n_traj, strict_mode=True
        )
        res_shift = sim.simulate_dynamics(
            t, initial_state=psi0 * 1.001, n_traj=n_traj, strict_mode=True
        )
    ...
    def test_si_7_detailed_balance(self, fmo_hamiltonian):
        ...
        sim = HopsSimulator(
            fmo_hamiltonian, temperature=temp, max_hierarchy=max(1, L_target - 2)
        )
    ...
    def test_si_11_spectral_normalization(self):
        ...
        logger.info(
            f"Test 11 (Integral): Calculated lambda = {integral:.2f}, Target = {lam}"
        )
    ...
    def test_si_12_markovian_recovery(self, fmo_hamiltonian):
        ...
        assert np.max(diff) < 1e-2, (
            f"Population increased significantly in Markovian limit: {np.max(diff)}"
        )
