...
class UnifiedFigures:
    ...
    def plot_quantum_dynamics(
        ...
        2 + min(n_sites, 4)  # Total population, coherences, and up to 4 site pops
    ...
    def plot_spectral_analysis(
        ...
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(
            2, 2, figsize=(self.figsize_base[0], self.figsize_base[1])
        )
        ...
        ax1.plot(
            ...
            "orange",
            linewidth=2,
            label="Solar Spectrum (AM1.5G)",
            alpha=0.7,
        )
        for i, T in enumerate(transmission_funcs):
            label = (
                "Transmission (Optimized)"
                if i == len(transmission_funcs) - 1
                else "Transmission (Initial)"
            )
            ax1.plot(
                wavelengths,
                T * np.max(solar_irradiance),
                linewidth=2,
                label=label,
                alpha=0.8,
            )
        ...
        if len(transmission_funcs) > 0:
            ...
            solar_irradiance * (1 - T_final) * R_opv

            ax3.plot(
                wavelengths,
                solar_irradiance,
                label="Incident Solar",
                linewidth=2,
                alpha=0.5,
            )
        ...
        if len(transmission_funcs) > 0:
            ...
            quantum_advantage = np.abs(
                np.gradient(T_final, wavelengths)
            )  # High gradient = potential quantum advantage
    ...
    def plot_optimization_results(
        self, param_history, performance_history, title="Optimization Progress"
    ):
        """
        ...
        for i in range(min(n_params, 4)):  # Limit to first 4 parameters
            if n_params == 1:
                axes[0, 0].plot(
                    generations, params_array, label=f"Parameter {i + 1}", linewidth=2
                )
            else:
                axes[0, 0].plot(
                    generations,
        ...
        for i in range(n_metrics):
            i if len(perf_array.shape) > 1 else 0
            ...
            axes[1, i].plot(
                ...
                color=self.colors.get(
                    list(self.colors.keys())[i % len(self.colors)], "blue"
                ),
            )
    ...
    def plot_eco_design_analysis(self, eco_df, title="Eco-Design Analysis"):
        ...
        if "biodegradability" in eco_df.columns and "pce_potential" in eco_df.columns:
            scatter = axes[0, 0].scatter(
                eco_df["biodegradability"],
                eco_df["pce_potential"],
        ...
        if "multi_objective_score" in eco_df.columns:
            ...
            axes[1, 0].hist(
                scores, bins=20, color=self.colors["opv"], alpha=0.7, edgecolor="black"
            )
        ...
        if all(col in eco_df.columns for col in ["biodegradability", "pce_potential"]):
            eco_threshold = eco_df[
                (eco_df["biodegradability"] > 0.7) & (eco_df["pce_potential"] > 0.12)
            ]
    ...
    def plot_quantum_advantage_analysis(
        ...
        temp_grid, dis_grid = np.meshgrid(
            temperatures[:: max(1, len(temperatures) // 10)],
        ...
        if len(temp_sensitivity) > 1 and len(disorder_sensitivity) > 1:
            ax4.boxplot(
                [temp_sensitivity, disorder_sensitivity],
                labels=["Temperature", "Disorder"],
            )
    ...
    def plot_all_results_summary(
        self, results_dict, title="Comprehensive Results Summary"
    ):
        """
        ...
        if all(k in results_dict for k in ["time_points", "populations", "coherences"]):
            ...
            ax1.plot(
                time_points, total_pop, "k-", linewidth=2, label="Total Population"
            )
            ax1_twin = ax1.twinx()
            ax1_twin.plot(
                time_points,
        ...
        if "pce" in results_dict and "etr" in results_dict:
            ...
            bars = ax2.bar(
                metrics,
                values,
                color=[self.colors["opv"], self.colors["psu"], self.colors["etr"]],
            )
        ...
        if all(
            k in results_dict
            for k in ["wavelengths", "solar_irradiance", "transmission"]
        ):
        ...
        for key in ["qfi", "entropy", "purity", "concurrence"]:
            if key in results_dict:
                if hasattr(results_dict[key], "__len__") and len(results_dict[key]) > 0:
                    quantum_metrics[key] = np.mean(
                        results_dict[key][-10:]
                    )  # Last 10 values average
                else:
        ...
        if "robustness" in results_dict:
            ...
            if (
                "temperature_sensitivity" in robustness
                and "disorder_sensitivity" in robustness
            ):
                ...
                ax6.plot(
                    range(len(temp_data)), temp_data, label="Temperature", linewidth=2
                )
        ...
        if performance_items:
            ...
            plt.setp(
                ax7.get_xticklabels(), rotation=45, ha="right", rotation_mode="anchor"
            )
    ...
    def save_data_to_csv(self, data, filename, subdir=None):
        ...
        if ext.lower() != ".csv":
            ...
            base_name = (
                base_name  # Remove .csv from base_name to add timestamp before it
            )
