...
CURRENT_NB = os.path.join(
    BASE_DIR, "quantum_coherence_agrivoltaics_analysis_refined.ipynb"
)
ARCHIVE_NB = os.path.join(
    BASE_DIR, "Archive/Code_old/quantum_coherence_agrivoltaics_analysis_refined.ipynb"
)
...
if already_has_class:
    ...
    if archive_cell_idx < 0:
        ...
        if archive_cell_idx > 0:
            ...
            if prev_cell["cell_type"] == "markdown" and "Biodegradability" in "".join(
                prev_cell["source"]
            ):
        ...
        usage_idx = find_cell_index(
            current_nb, "biodegradability_analyzer = BiodegradabilityAnalyzer("
        )
        if usage_idx < 0:
            print(
                "  [ERROR] Could not find BiodegradabilityAnalyzer usage in current notebook!"
            )
        else:
            ...
            print(
                f"  [OK] Inserted BiodegradabilityAnalyzer class at cell index {usage_idx}"
            )
            if archive_md_cell:
                print("  [OK] Also inserted markdown header cell")
...
for cell in current_nb["cells"]:
    ...
    if "FMO_Site_Energies.pdf" in joined:
        ...
        for line in source:
            ...
            if "plt.savefig" in line and "fmo_hamiltonian_data.csv" not in joined:
                new_source.append(
                    f'{indent}pd.DataFrame(fmo_hamiltonian).to_csv(os.path.join(DATA_DIR, "fmo_hamiltonian_data.csv"))\n'
                )
                new_source.append(
                    f'{indent}pd.DataFrame(fmo_energies).to_csv(os.path.join(DATA_DIR, "fmo_site_energies.csv"))\n'
                )
    ...
    # --- 2b. PAR_Transmission: dashed lines + smaller legend ---
        ...
        for line in source:
            ...
            if (
                "plt.plot(wavelengths, T_dusty" in line
                and "linestyle" not in line
                and "linewidth" in line
            ):
            ...
            if "plt.savefig" in line and "solar_irradiance_data.csv" not in joined:
                new_source.append(
                    f"{indent}pd.DataFrame({{'wavelength': wavelengths, 'irradiance': solar_irradiance}}).to_csv(os.path.join(DATA_DIR, \"solar_irradiance_data.csv\"))\n"
                )

        cell["source"] = new_source
        print(
            "  [OK] PAR_Transmission: Applied dashed lines + smaller legend + data export"
        )
    ...
    # --- 2c. Global_Reactivity_Indices.pdf: 2x2 grid, remove empty panels ---
        ...
        if plot_start_idx != -1:
            ...
            cell["source"] = new_source
            print(
                "  [OK] Global_Reactivity_Indices.pdf: Applied 2x2 grid layout + data export"
            )
                ...
                if match:
                    ...
                    if filename not in joined:
                        if "Quantum_Advantage_in_Energy_Transfer.pdf" in line:
                            temp_source.append(
                                f"{indent}pd.DataFrame({{'site': sites, 'pop_filtered': populations_filtered_avg[-1], 'pop_broad': populations_broad_avg[-1]}}).to_csv(os.path.join(DATA_DIR, \"{filename}\"))\n"
                            )
                            modified = True
                        elif "Pareto_Front__PCE_vs_ETR_Trade_off.pdf" in line:
                            temp_source.append(
                                f"{indent}pd.DataFrame({{'pce': results_df['pce'], 'etr': results_df['etr_mean']}}).to_csv(os.path.join(DATA_DIR, \"{filename}\"))\n"
                            )
                            modified = True
                        elif "ETR_Uncertainty_Distribution.pdf" in line:
                            temp_source.append(
                                f"{indent}pd.DataFrame({{'etr_samples': mc_results['etr']['samples']}}).to_csv(os.path.join(DATA_DIR, \"{filename}\"))\n"
                            )
                            modified = True
        if modified:
