...
def test_hops_simulator_initialization(config):
    ...
    logger.info(
        f"HopsSimulator initialized: T={simulator.temperature} K, mesohops={simulator.use_mesohops}"
    )
...
def test_hops_simulator_simulation_stub():
    ...
    logger.info(
        f"Simulation result keys: {list(results.keys())}, pop shape: {results['populations'].shape}"
    )
