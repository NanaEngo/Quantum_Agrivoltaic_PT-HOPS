...
ax.axhspan(
    285, 300, alpha=0.3, color=colors["optimal"], label="Optimal range (285-300 K)"
)
...
ax.axvline(
    x=mu, color=colors["fit"], linestyle="--", linewidth=2, label=f"Mean: {mu:.3f}"
)
ax.axvspan(
    mu - 1.96 * sigma, mu + 1.96 * sigma, alpha=0.2, color=colors["fit"], label="95% CI"
)

# Mark threshold from manuscript (η > 0.15)
ax.axvline(x=0.15, color="red", linestyle=":", linewidth=2, label="Threshold (0.15)")
...
ax.annotate(
    f"CV = {cv:.1f}%", xy=(0.25, 0.7 * max(pdf_fit)), fontsize=9, fontweight="bold"
)
...
print(
    f"  Temperature range: {df['temperature_k'].min():.1f} - {df['temperature_k'].max():.1f} K"
)
