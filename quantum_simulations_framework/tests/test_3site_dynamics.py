...
def test_3site_simulation_with_config():
    ...
    # 4. Setup simulator using laptop parameters
    ...
    print(
        f"   SBD: ENABLED ({sbd_bundles} bundles/site) — required at L>=2 to limit hierarchy size"
    )
    print(
        f"   Estimated hierarchy modes: {n_sites * (K + sbd_bundles)} (vs {n_sites * (K + 12)} without SBD)"
    )
    ...
    # 6. Validations
    t_axis_out = results["t_axis"]
    pops = results["populations"]
    print(f"   Populations shape: {pops.shape}")
    print(f"   Time axis shape: {t_axis_out.shape}")

    # Check dimensionality
    assert H3.shape == (3, 3)
    assert pops.shape == (len(t_axis_out), 3), (
        f"Population shape {pops.shape} does not match time axis {len(t_axis_out)}"
    )
