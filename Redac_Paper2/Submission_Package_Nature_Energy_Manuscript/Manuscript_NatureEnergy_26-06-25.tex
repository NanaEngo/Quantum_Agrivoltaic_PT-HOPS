...
A non-Hermitian \SI{9}{\times}\SI{9}{} dressed Hamiltonian (8 FMO sites $+$ 1 plasmon mode) captures the strong coupling regime ($g_0 > \SI{200}{\per\cm}$ at mode volumes $V < \SI{0.36}{\nm\cubed}$, with $g_0 = \SI{134}{\per\cm}$ at the optimal $V = \SI{1.2}{\nm\cubed}$).
...
At the sub-nanometer mode volumes accessible in NPoM cavities ($V_{\mathrm{mode}} < \SI{1}{\nm\cubed}$), $g_0$ ranges from \SI{120}{} to over \SI{300}{\per\cm}, and the collective coupling $g_{\mathrm{eff}} \sim \SI{2000}{\per\cm}$ places the system firmly in the strong coupling regime ($g_{\mathrm{eff}} > \kappa$, where $\kappa \approx \SI{800}{\per\cm}$ is the plasmon linewidth).
...
At \SI{1000}{\W\per\m\squared}, the transmission drops to \SI{44}{\percent} of baseline (\Cref{fig:npq_switch}b), providing dynamic protection against exciton overload.
...
\begin{equation}
  ...
  \label{eq:phi_global}
\end{equation}
yielding $\Phi_{\mathrm{FT}}^{\mathrm{global}} = 0.971$ at $\alpha = 0.01$, $\Phi_{\mathrm{FT}}^{\mathrm{NPoM}} = 0.077$ --- a result that resolves the apparent contradiction between NPoM-induced transport suppression and sustained photosynthetic productivity across the full canopy.
...
This is \SI{1.9}{\times} shorter than the 8.0~yr payback for the unsubsidized 1-ha tomato scenario, demonstrating that high-value crop selection and blended-finance structurally transform the investment case.
...
\textbf{(b)} Exciton population dynamics under dual-band filtered excitation at \SI{295}{\kelvin}, showing the competitive loading of the plasmon mode (Site~9) versus the FMO antenna and trapping sites.
