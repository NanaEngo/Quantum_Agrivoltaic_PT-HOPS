...
def run_comprehensive_test():
    ...
    CSVDataStorage(output_dir="./simulation_data")
    FigureGenerator(figures_dir="./figures")
    ...
    try:
        ...
        if "qfi" in results:
            ...
            print(
                f"   ✓ QFI range: {results['qfi'].min():.4f} - {results['qfi'].max():.4f}"
            )
        if "entropy" in results:
            dynamics_data["entropy"] = results["entropy"]
            print(
                f"   ✓ Entropy range: {results['entropy'].min():.4f} - {results['entropy'].max():.4f}"
            )
        if "purity" in results:
            dynamics_data["purity"] = results["purity"]
            print(
                f"   ✓ Purity range: {results['purity'].min():.4f} - {results['purity'].max():.4f}"
            )

        import pandas as pd

        pd.DataFrame(dynamics_data).to_csv(
            dynamics_csv, index=False, float_format="%.6e"
        )
    ...
    try:
        ...
        print("   ✓ Fukui functions calculated")
        ...
        ax.set_title(
            f"Fukui Functions - Biodegradability Score: {biodegradability_score:.3f}"
        )
        ax.legend()
    ...
    try:
        ...
        print(
            f"   ✓ Carbon footprint: {lca_results['carbon_footprint_gco2eq_per_kwh']:.2f} gCO2eq/kWh"
        )
        print(
