...
class TechnoEconomicModel:
    ...
    def __init__(
        ...
        crop_price_per_kg: float = DEFAULT_CROP_PRICE_KG,
    ):
    ...
    def evaluate_project_viability(
        ...
        pce: float,
        etr: float,
        base_crop_yield_kg_per_ha: float = DEFAULT_BASE_CROP_YIELD,
        capex_per_kw: float = DEFAULT_CAPEX_KW,
        opex_per_kw_yr: float = DEFAULT_OPEX_KW_YR,
    ) -> Dict[str, Any]:
        """
        ...
        """
        # Convert Area
        total_area_m2 = area_hectares * HECTARE_TO_M2
        pv_area_m2 = total_area_m2 * pv_coverage_ratio

        # 1. PV Energy Yield Calculation
        # Assuming SOLAR_IRRADIANCE_ANNUAL as baseline solar irradiance
        annual_energy_kwh = SOLAR_IRRADIANCE_ANNUAL * pv_area_m2 * pce
        system_capacity_kw = (
        ...
        crop_yield_kg_yr = base_crop_yield_kg_per_ha * area_hectares * effective_light_ratio * SHADING_BOOST_FACTOR
