...
from quantum_advantage_calculator import (
...
def solar_spectrum_am15g(wavelengths):
    ...
    irradiance = (
        irradiance
        * 1000
        / trapezoid(irradiance, wavelengths)
...
def load_simulation_params(
    param_file_path="./data_input/quantum_agrivoltaics_params.json",
):
    """
    ...
    try:
        ...
        print(
            f"Parameter file not found at {param_file_path}, using default parameters"
        )
        ...
        return {
            ...
            "fmo_hamiltonian_params": {"include_reaction_center": False},
            "opv_params": {"opv_bandgap": 1.4, "opv_absorption_coeff": 1.0},
            "quantum_metrics": {
                "calculate_qfi": True,
...
def create_solar_spectrum(wavelength_min=None, wavelength_max=None, n_points=None):
    ...
    solar_irradiance = (
        solar_irradiance
        * 1000
        / np.trapezoid(solar_irradiance, wavelengths)
        * (wavelengths[1] - wavelengths[0])
    )
...
def calculate_etrs_for_transmission(
    transmission_func,
    wavelengths,
    solar_irradiance,
    fmo_hamiltonian,
    ...
    np.mean(transmitted_spectrum[par_mask]) if np.any(par_mask) else 0

    # Create a modified initial state based on transmission
    # For the FMO complex, different sites absorb at different wavelengths
    fmo_simulator = QuantumDynamicsSimulator(
        fmo_hamiltonian, temperature=295, dephasing_rate=10
    )

    # Modify initial state based on transmission
    initial_state = np.zeros(
        (fmo_simulator.n_sites, fmo_simulator.n_sites), dtype=complex
    )
    ...
    for i in range(fmo_simulator.n_sites):
        initial_state[i, i] = site_weights[i] * (
            0.5 + 0.5 * np.mean(transmission_values[par_mask])
        )  # Scale based on average transmission
    ...
    _, _, populations, _, _, _, _, _, _, _, _ = fmo_simulator.simulate_dynamics(
        initial_state=initial_state, time_points=time_points
    )
...
def run_complete_analysis():
    ...
    print(
        f"Using {n_processes} processes out of {total_cores} available cores (default logic: nproc-4 if nproc>=8; else nproc-2)"
    )
    ...
    print(
        f"  Site energies range: {np.min(site_energies):.1f} to {np.max(site_energies):.1f} cm⁻¹"
    )
    ...
    print("  Quantum dynamics simulator initialized")
    ...
    ) = quantum_sim.simulate_dynamics(
        time_points=np.linspace(0, 500, 100)  # fs
    )

    print("  Quantum dynamics simulation completed")
    print(
        f"    Time evolution: {len(time_points)} points from {time_points[0]:.1f} to {time_points[-1]:.1f} fs"
    )
    ...
    _, _, etr_per_photon_vals = quantum_sim.calculate_etr(
        populations, time_points
    )  # Use the new method
    ...
    print(
        f"  Solar spectrum created: {len(wavelengths)} points from {wavelengths[0]:.0f} to {wavelengths[-1]:.0f} nm"
    )
    ...
    print(
        f"    PSU response range: {np.min(agrivoltaic_model.R_psu):.3f} to {np.max(agrivoltaic_model.R_psu):.3f}"
    )
    ...
    spec_opt = SpectralOptimizer(
        agrivoltaic_model, quantum_sim, n_processes=n_processes
    )
    ...
    try:
        ...
        if opt_results["success"]:
            print("  Spectral optimization completed successfully")
            ...
            print(
                f"    Optimal parameters: {opt_results['optimal_params'][:6]}..."
            )  # Show first few
        else:
            print("  Spectral optimization did not converge, using initial parameters")
        ...
        print(
            f"  Spectral optimization encountered an error: {str(e)}, using initial parameters"
        )
        # Calculate performance with initial parameters as fallback
        pce_initial = spec_opt.calculate_pce(T_initial)
        etr_initial = spec_opt.calculate_etr(T_initial)
    ...
    print("  Eco-design analyzer initialized")

    # Find eco-friendly candidates
    eco_candidates = eco_analyzer.find_eco_friendly_candidates(
        min_biodegradability=0.7, min_pce_potential=0.12
    )
    ...
    if eco_candidates:
        ...
        print(
            f"      Multi-objective score: {eco_candidates[0]['multi_objective_score']:.3f}"
        )
    ...
    if eco_candidates:
        ...
        data_storage.save_eco_analysis_to_csv(
            eco_candidates, filename="eco_design_analysis"
        )
    ...
    print("  Figure generator initialized")

    # Generate quantum dynamics figure
    fig_gen.plot_quantum_dynamics(
        time_points, populations, coherences, qfi_values, etr_time
    )

    # Generate extended quantum metrics figure
    fig_gen.plot_quantum_metrics_extended(
        time_points,
        entropy_values,
        purity_values,
        linear_entropy_values,
    ...
    fig_gen.plot_robustness_analysis(robustness_data, title="Robustness Analysis")
...
def run_complete_analysis_with_params():
    ...
    total_cores = os.cpu_count()
    if (
        "parallelization_params" in params
        and params["parallelization_params"]["nproc"] is not None
    ):
    ...
    print(
        f"  Site energies range: {min(fmo_energies):.1f} to {max(fmo_energies):.1f} cm⁻¹"
    )
    ...
    quantum_sim = QuantumDynamicsSimulator(
        fmo_hamiltonian, temperature=temp, dephasing_rate=dephasing
    )
    ...
    print(
        f"  Running quantum dynamics simulation for {max_time} fs with {n_time_points} time points..."
    )
    ...
    (
        ...
        multipartite_ent_values,
        pairwise_concurrence_values,
    ) = quantum_sim.simulate_dynamics(time_points=time_points)
    ...
    print(
        f"  Solar irradiance range: {solar_irradiance.min():.3f} - {solar_irradiance.max():.3f} mW/cm²/nm"
    )
    ...
    try:
        ...
        if result.success:
            ...
            opt_results = {
                ...
                "final_transmission": agrivoltaic_model.calculate_spectral_transmission(
                    result.x
                ),
                "pce": spec_opt.calculate_pce(
        ...
        # Calculate performance with initial parameters as fallback
        pce_initial = spec_opt.calculate_pce(T_initial)
        etr_initial = calculate_etrs_for_transmission(
            T_initial,
            wavelengths,
            solar_irradiance,
            fmo_hamiltonian,
    ...
    try:
        ...
        quantum_adv_fig = fig_gen.plot_quantum_advantage_analysis(
            temperatures,
            robustness_data["temperature_sensitivity"],
            disorder_strengths,
            robustness_data["disorder_sensitivity"],
            title="Quantum Advantage Analysis",
        )
        fig_gen.save_figure(quantum_adv_fig, "quantum_advantage_analysis.png")
    except ImportError:
        print(
            "  Quantum advantage calculator not available, skipping quantum advantage analysis"
        )
    ...
    summary_fig = fig_gen.plot_all_results_summary(
        results_dict, title="Comprehensive Results Summary"
    )
    fig_gen.save_figure(summary_fig, "comprehensive_summary.png")

    print("  All figures generated and saved to figures/ directory")
    ...
    print(
        f"  SPCE (initial → optimized): {spce_initial:.4f} → {opt_results['spce']:.4f}"
    )
