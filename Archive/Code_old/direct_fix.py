...
def direct_fix():
    ...
    # Read the file
    with open(
        "/media/taamangtchu/MYDATA/Github/Comparative-study-of-the-Anderson-model-in-weak-and-strong-interaction-regimes/Redac_Paper1/quantum_coherence_agrivoltaics_analysis.ipynb",
        "r",
    ) as f:
        content = f.read()
    ...
    content = re.sub(
        r'"source": \[.*?\](?:,)?', fix_source_quotes, content, flags=re.DOTALL
    )
    ...
    content = re.sub(
        r'([^\\])"([^",\]}\s])', r"\1\"\2", content
    )  # Do it twice for overlapping matches
    ...
    try:
        ...
        if all(field in data for field in required_fields):
            ...
            with open(
                "/media/taamangtchu/MYDATA/Github/Comparative-study-of-the-Anderson-model-in-weak-and-strong-interaction-regimes/Redac_Paper1/quantum_coherence_agrivoltaics_analysis_fixed.ipynb",
                "w",
            ) as f:
        ...
        with open(
            "/media/taamangtchu/MYDATA/Github/Comparative-study-of-the-Anderson-model-in-weak-and-strong-interaction-regimes/Redac_Paper1/direct_fix_attempt.json",
            "w",
        ) as f:
            f.write(content)
