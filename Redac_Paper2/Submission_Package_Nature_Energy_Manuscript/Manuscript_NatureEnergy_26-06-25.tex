...
The coherence lifetime $\tau_c$, defined as the $1/e$ decay time of the $l_1$-norm coherence $C_{l_1}(t) = \sum_{i\neq j} |\rho_{ij}(t)|$~\cite{Baumgratz2014}, extends from \qty{280(25)}{\fs} (broadband) to \qty{420(35)}{\fs} (filtered) --- a \qty{50}{\percent} increase.
...
\Cref{tab:comparative_summary} summarizes the exciton trapping yields and population metrics across all four simulated configurations, including the \qty{77}{\kelvin} cryogenic benchmark.

\input{table_comparative_4runs}

\Cref{tab:comparative_summary} reveals three salient features of the NPoM-coupled exciton dynamics. First, the forward transfer yield is suppressed by more than \qty{90}{\percent} across all room-temperature NPoM configurations relative to the pristine FMO baseline ($\Phi_{\mathrm{FT}} = \num{0.98}$). The origin of this suppression is the strong-coupling hybridization: even at the optimal volume $V_{\mathrm{mode}} = \qty{1.2}{\nm\cubed}$, the plasmon mode (Site~9) retains \qty{83}{\percent} of the population at steady state, leaving only \qty{12}{\percent} on the FMO antenna sites and less than \qty{0.4}{\percent} on the trapping sites~3 and~4.

Second, reducing the temperature to \qty{77}{\kelvin} approximately doubles the trapping yield to $\Phi_{\mathrm{FT}} = 0.1685$, a \qty{2.1}{\times} enhancement over the equivalent room-temperature configuration ($\Phi_{\mathrm{FT}} = 0.0799$ at $V_{\mathrm{mode}} = \qty{1.2}{\nm\cubed}$). This cryogenic enhancement is attributable to the suppression of thermal phonon-induced decoherence: the frozen bath reduces the pure-dephasing rate of the excitonic coherence, permitting a larger fraction of the wavepacket to navigate to the reaction center before being captured by the plasmon sink. The maximum trapped population on sites~3+4 rises by approximately \qty{45}{\percent} (precisely \qty{44.7}{\percent}, from $\num{3.8e-3}$ to $\num{5.5e-3}$), consistent with a longer effective transport length at low temperature.

All NPoM-on runs exhibit perfect trace conservation ($\Tr\rho = \num{1.0000 \pm 0.0000}$), validating the numerical stability of the PT-HOPS/SBD propagator. The $V=\qty{0.8}{\nm\cubed}$ configuration was run with $n=100$ disorder realizations; the $V=\qty{1.2}{\nm\cubed}$ configuration was validated with $n=20$ trajectories ($\Phi_{\mathrm{FT}} = 0.0799$, within \qty{0.6}{\percent} of the $n=2$ estimate of $0.0804$); and the \qty{77}{\kelvin} cryogenic run used $n=2$ trajectories. The baseline NPoM-off run is shown for completeness but uses the 8-site FMO Hamiltonian and should not be compared quantitatively with the converged NPoM-on data.
...
\textbf{Pathogen detection} is achieved by co-deploying Nitrogen-Vacancy (NV) center nanodiamond sensors in the sentinel probe. These solid-state quantum sensors provide extreme biomagnetic sensitivity, detecting paramagnetic bacterial metabolites to identify pathogens like \textit{Pseudomonas fluorescens} at pre-symptomatic stages (LOD = \qty{0.8}{\nano\mol\per\L}) before macroscopic tissue damage occurs~(\cref{SI-sec:nv_diamond}). We note that while these limits of detection are established under controlled laboratory environments, the complex agricultural matrix (soil runoff, dust accumulation, and biological debris) introduces substantial spectral noise that can degrade the effective in-field sensitivity. In our system, this matrix effect is mitigated by the protective graphene barrier in the NPoM cavities and the algorithmic drift corrections of the \textit{DynamicCalibrator}.
...
At the sensor level, long-term baseline drift from biofouling and thermal cycling is corrected by a \textit{DynamicCalibrator} module: an exponential moving average of daily reference readings ($\alpha = 0.1$) produces a multiplicative correction factor, and an alarm is raised when drift exceeds \qty{20}{\percent} of the initial baseline, indicating that panel cleaning is required.
...
We verified this by applying the protocol to a minimal three-site excitonic trimer, obtaining $\eta_{\mathrm{FT}} = \num{0.18 \pm 0.03}$ (\Cref{SI-sec:3site}), consistent with the quantum phase synchronization mechanism identified in photosynthetic antennas~\cite{Zhu2024}.
...
A hybrid quantum-classical signal processing pipeline --- combining Matrix Product State (MPS) denoising with quantum kernel ridge regression (QKRR)~\cite{Stoudenmire2016,Havlicek2019} --- further enables the digital twin to extract pre-symptomatic stress signatures from the noisy SERS and CQD fluorescence telemetry. The MPS compressor acts as a nonlinear low-pass filter that suppresses greenhouse noise (thermal fluctuations, biofouling, dust) while preserving the sharp vibronic features at \qty{180}{\per\cm} and \qty{1145}{\per\cm} that classical Savitzky--Golay filters tend to smear~(\Cref{SI-sec:qml}). The cleaned spectra are projected through a quantum-inspired kernel for anomaly scoring, issuing early warnings that allow preemptive agronomic intervention before macroscopic symptoms appear --- all within a \qty{3.7}{\milli\second} compute budget on an ARM Cortex-M4 IoT node.
...
The BB84 simulation assumes a buried single-mode fiber link; free-space implementation in dusty field conditions would require additional atmospheric correction.
The \qty{1}{\percent} sentinel fraction is a design parameter; field optimization will require empirical calibration of the SERS LODs under actual fouling conditions. The \textit{DynamicCalibrator} compensates for biofouling algorithmically but imposes a recurring cleaning OPEX; zwitterionic polymer coatings~\cite{Kumar2022,Do2025} offer a hardware-level path to eliminate both the algorithmic drift correction and the manual maintenance cycle.

\textbf{Outlook.} The integration of quantum coherence engineering with agricultural sustainability opens a new axis for agrivoltaic design. Future research should prioritize three main development directions: materials hardware optimization, algorithmic routing, and standardization.

First, on the hardware and materials scale, future work must extend the quantum model to full multi-exciton dynamics under realistic solar spectra, accompanied by experimental validation via two-dimensional electronic spectroscopy (2DES) with spatial light modulator pulse shaping~\cite{Gunthardt2023,Sebastian2024}. Field trials of graphene-based NPoM coatings on greenhouse cladding with the CQD sentinel network are required to evaluate sensor performance under actual biofouling conditions. To minimize transport penalties while preserving SERS sensitivity, alternative plasmonic architectures should be explored, such as dielectric nanoantennas (\ce{Si}, \ce{TiO2}) supporting low-loss Mie resonances~\cite{Caldarola2015} or nanocavity-free substrates. Furthermore, the development of zwitterionic polymer coatings~\cite{Kumar2022,Do2025} that preserve \qty{90}{\percent} quantum coherence after \qty{60}{\day} field immersion offers a path to replace algorithmic calibration with hardware-level biofouling resistance (\cref{SI-sec:zwitterionic}). This can be supplemented by passive thermal micro-shielding to reduce Stern--Volmer thermal drift (\cref{SI-sec:thermal_shielding}), UiO-66 Zr-based quantum metal-organic frameworks (qMOFs) for coherent contaminant remediation (\cref{SI-sec:mof}), NV-diamond relaxometry for sub-nanomolar pathogen detection (\cref{SI-sec:nv_diamond}), and quantum fertilizer biostimulation nanoparticles to enhance chlorophyll yields (\cref{SI-sec:quantum_fertiliser}).

Second, on the algorithmic and security front, we advocate for the integration of the real-time agrivoltaic digital twin with the Quantum Approximate Optimization Algorithm (QAOA)~\cite{Farhi2014,AlSagri2025} to schedule water-energy-food resources dynamically under fluctuating solar flux (\cref{SI-sec:qaoa}). This quantum-inspired or quantum-native scheduling layer can be verified and secured by a data-sovereignty protocol~\cite{Zafar2025,AgriFLChain2025} built directly on top of the BB84 QKD fiber link (\cref{SI-sec:datasovereignty}), ensuring that smallholder cooperatives retain ownership and monetization rights over the hyperspectral agronomic datasets they generate. These digital twins can also incorporate regional hydrogeological tracking via transportable absolute quantum gravimeters~\cite{Stray2022,Menoret2018} to map groundwater recharge and bridge the crop microclimate models with satellite geodesy (\cref{SI-sec:gravimetry}), all validated through Monte Carlo uncertainty propagation in the LCA.

Third, to prevent these technological advances from widening the ``quantum divide,'' we strongly advocate for the establishment of the Global Quantum Agriculture Standard (GQAS). GQAS is proposed as a six-pillar certification framework --- covering QKD security, data sovereignty, sensor detection limits, quantum fidelity, latency, and lifecycle sustainability (\cref{SI-sec:gqas}) --- to guarantee that affordable, subsidized sensor networks are legally and commercially recognized on premium export markets, providing smallholder cooperatives with a verified, blockchain-anchored quality seal that translates environmental gains into tangible economic value.
...
Wind speed within the greenhouse is modeled at a static representative value of $u = \qty{0.5}{\m\per\s}$, corresponding to a typical $\qty{10}{\percent}$ reduction of average external wind velocities.
...
Crucially, this algorithmic correction is paired with a physical micro-shielding mechanism (temperature-stabilization layer) that insulates the sensitive CQD and NPoM probes against diurnal thermal excursions and soil humidity. This shielding reduces the Stern--Volmer baseline thermal drift by a factor of $(1-\xi_{\mathrm{shield}})$ (where $\xi_{\mathrm{shield}} = 0.85$), ensuring robust calibration under harsh field conditions.
A daily reference measurement (clean water blank) updates the EMA; a drift alarm fires when the fractional change in the EMA exceeds \qty{20}{\percent} of the initial baseline, signalling that OPV panel cleaning is required.
