...
def create_fixed_notebook(notebook_path):
                ...
                for line in cell["source"]:
                    if "workers=mp.cpu_count()" in line:
                        line = line.replace(
                            ...
                            "workers=1  # Use single process to avoid issues in Jupyter notebooks",
                        )
    ...
    for cell in cells:
        if "source" in cell:
            ...
            if (
                "transmission_profiles = [" in source_text
                and "Optimized" in source_text
            ):
                ...
                while i < len(source):
                    ...
                    if "transmission_profiles = [" in line:
                        ...
                        i += 1
                        while i < len(source) and not any(
                            x in source[i]
                            for x in [
                                "{'name': 'Optimized'",
                                "{'name': 'Broad Bandpass'",
            ...
            for line in source:
                if "def calculate_qfi" in line:
                    ...
                    fixed_line = line.replace(
                        "> 1e-12",
                        "> 1e-8  # More conservative threshold to prevent numerical instabilities",
                    )
                    ...
                    new_source.append(
                        "        # Apply upper bound to prevent extremely large QFI values\n"
                    )
                    new_source.append(
                        "        qfi = min(qfi, 1e10)  # Cap extremely large values\n"
                    )
                    new_source.append(line)
