...
try:
    ...
    class EnvironmentalFactors:
        ...
        def combined_environmental_effects(self, *args, **kwargs):
            ...
            return (
                [base_pce] * len(time_range),
                [base_etr] * len(time_range),
                [0.1] * len(time_range),
            )
...
class BiodegradabilityAnalyzer:
    ...
    def calculate_biodegradability_score(self, weights=None):
        ...
        score = (
            ...
            * min(1.0, global_softness * 10)  # Normalize softness
...
def run_complete_simulation(n_processes=None):
    ...
    eco_analyzer = EcoDesignAnalyzer(
        agrivoltaic_model=ag_model, quantum_simulator=qd_sim
    )
    ...
    print(
        f"    Time points: {len(time_points)} from {time_points[0]:.0f} to {time_points[-1]:.0f} fs"
    )
    ...
    try:
        ...
        print(
            f"    Fukui functions calculated - f+ max: {np.max(f_plus):.3f}, f- max: {np.max(f_minus):.3f}"
        )
        ...
        molecular_data = {
            "pm6_derivative": {
                "biodegradability_score": min(
                    0.8, biodegrad_score + 0.1
                ),  # Adjusted for PM6
                "b_index": 72.0,  # From the published research
                "pce": 0.15,
            },
            "y6_bo_derivative": {
                "biodegradability_score": max(
                    0.4, biodegrad_score - 0.1
                ),  # Adjusted for Y6-BO
        ...
        print(
            f"    PM6 derivative biodegradability: {molecular_data['pm6_derivative']['biodegradability_score']:.3f}"
        )
        print(
            f"    Y6-BO derivative biodegradability: {molecular_data['y6_bo_derivative']['biodegradability_score']:.3f}"
        )
    ...
    print(
        f"  Temperature coefficient (OPV): {env_factors.temperature_coefficient_opv} per K"
    )
    print(
        f"  Temperature coefficient (PSU): {env_factors.temperature_coefficient_psu} per K"
    )
    ...
    temperatures = (
        298
        + 5 * np.sin(2 * np.pi * time_days / 30)
        + np.random.normal(0, 3, size=time_days.shape)
    )  # K
    humidity_values = (
        0.5
        + 0.2 * np.sin(2 * np.pi * time_days / 20)
        + np.random.normal(0, 0.05, size=time_days.shape)
    )  # 0-1
    ...
    data_storage.save_biodegradability_analysis_to_csv(
        biodegrad_list, filename="biodegradability_analysis"
    )
    ...
    print(
        f"  - Agrivoltaic coupling: {len(time_points_ag)} time points, {opv_pops.shape[1]} OPV sites, {psu_pops.shape[1]} PSU sites"
    )
    print(
        f"  - Spectral optimization: PCE={opt_results['pce']:.3f}, ETR={opt_results['etr']:.3f}, SPCE={opt_results['spce']:.3f}"
    )
