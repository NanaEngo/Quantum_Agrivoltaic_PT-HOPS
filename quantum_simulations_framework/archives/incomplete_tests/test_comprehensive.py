...
from core.hamiltonian_factory import (
...
class TestParameterConsistency:
    YAML_PATH = Path(__file__).parent.parent / "laptop_parameters.yaml"
    ...
    def test_hierarchy_depth(self):
        # In laptop mode, L_max is typically small (e.g., 3)
        # We just verify it exists and is valid
        assert "L_max" in self.cfg["dynamics"]
        assert self.cfg["dynamics"]["L_max"] > 0

    def test_production_consistency(self):
        """Verify that production parameters.yaml matches core constants."""
        prod_config = Path(__file__).parent.parent / "parameters.yaml"
        with open(prod_config, "r") as f:
            pcfg = yaml.safe_load(f)
        assert pcfg["dynamics"]["L_max"] == DEFAULT_MAX_HIERARCHY

    def test_matsubara_truncation(self):
        assert self.cfg["dynamics"]["matsubara_truncation"] == DEFAULT_N_MATSUBARA

    def test_temperature(self):
        assert self.cfg["bath"]["temperature"] == DEFAULT_TEMPERATURE

    def test_reorganization_energy(self):
        assert (
            self.cfg["bath"]["reorganization_energy"] == DEFAULT_REORGANIZATION_ENERGY
        )

    def test_drude_cutoff(self):
        assert self.cfg["bath"]["drude_cutoff"] == DEFAULT_DRUDE_CUTOFF

    def test_vibronic_frequencies_count(self):
        n_vib = len(DEFAULT_VIBRONIC_FREQUENCIES)
        assert len(self.cfg["bath"]["vibronic_frequencies"]) == n_vib
        assert len(DEFAULT_VIBRONIC_FREQUENCIES) == n_vib
    ...
    def test_huang_rhys_factors_count(self):
        n_vib = len(DEFAULT_HUANG_RHYS_FACTORS)
        assert len(self.cfg["bath"]["huang_rhys_factors"]) == n_vib
        assert len(DEFAULT_HUANG_RHYS_FACTORS) == n_vib
    ...
    def test_vibronic_damping_is_list(self):
        ...
        assert isinstance(damp, list), (
            "vibronic_damping must be a list in parameters.yaml"
        )
        assert len(damp) == len(DEFAULT_VIBRONIC_DAMPING)

    def test_time_step(self):
        # Verification matches the active constant or a physically valid range
        assert self.cfg["dynamics"]["time_step"] >= 0.1

    def test_pulse_fwhm(self):
        assert self.cfg["pulse"]["fwhm"] == PULSE_FWHM
...
class TestHamiltonianFactory:
    def test_7site_shape(self, fmo7):
        H, e = fmo7
        n = len(FMO_SITE_ENERGIES_7)
        assert H.shape == (n, n)
        assert len(e) == n

    def test_fmo7_size(self):
        H, e = create_fmo_hamiltonian(include_reaction_center=False)
        n = len(FMO_SITE_ENERGIES_7)
        assert H.shape == (n, n)
        assert len(e) == n
    ...
    def test_spectral_density_positive_omega(self):
        ...
        J = spectral_density_drude_lorentz(
            omega,
            DEFAULT_REORGANIZATION_ENERGY,
            DEFAULT_DRUDE_CUTOFF,
            DEFAULT_TEMPERATURE,
...
class TestHopsSimulatorFallback:
    ...
    def test_simulator_type_string(self, fallback_sim):
        assert (
            "fallback" in fallback_sim.simulator_type.lower()
            or "QuantumDynamics" in fallback_sim.simulator_type
        )
    ...
    def test_populations_key_present(
        self, fallback_sim, short_time, initial_state_site1
    ):
        result = fallback_sim.simulate_dynamics(short_time, initial_state_site1)
    ...
    def test_populations_shape(self, fallback_sim, short_time, initial_state_site1):
        ...
        assert pops.ndim == 2
        assert pops.shape[1] == initial_state_site1.shape[0]

    def test_populations_non_negative(
        self, fallback_sim, short_time, initial_state_site1
    ):
        result = fallback_sim.simulate_dynamics(short_time, initial_state_site1)
        assert np.all(result["populations"] >= -1e-6), (
            "Populations must be non-negative"
        )

    def test_populations_sum_conserved(
        self, fallback_sim, short_time, initial_state_site1
    ):
        result = fallback_sim.simulate_dynamics(short_time, initial_state_site1)
    ...
    def test_initial_excitation_site1(
        self, fallback_sim, short_time, initial_state_site1
    ):
    ...
    def test_no_double_fallback_init(self, fmo7):
        ...
        assert call_count["n"] == 1, (
            f"_init_fallback called {call_count['n']} times, expected 1"
        )
...
class TestCSVDataStorage:
    ...
    def test_config_hash_in_filename(self, tmp_path):
        ...
        cfg = {"dynamics": {"L_max": DEFAULT_MAX_HIERARCHY}}
...
class TestFigureGenerator:
    def test_creates_directory(self, tmp_path):
        FigureGenerator(figures_dir=str(tmp_path / "figs"))
...
class TestAuditConvergenceLogic:
    ...
    def test_identical_results_detected(self):
        """Relative hierarchy identical populations must be flagged as fake data."""
        pops = np.random.rand(100, 7)
        L_target = DEFAULT_MAX_HIERARCHY
        results = {L_target - 1: pops.copy(), L_target: pops.copy()}
        assert np.allclose(results[L_target - 1], results[L_target], atol=1e-12), (
    ...
    def test_shape_mismatch_detected(self):
        ...
        results = {
            L - 2: np.random.rand(100, 7),
            L - 1: np.random.rand(95, 7),  # different length
            L: np.random.rand(100, 7),
        }
        shapes = {l: results[l].shape for l in [L - 2, L - 1, L]}
    ...
    def test_convergence_threshold_check(self):
        """Relative hierarchy MAE < threshold must pass; above must warn."""
        threshold = 1e-6
        # Converged case
        p_prev = np.random.rand(50, 7)
        p_curr = p_prev + np.random.rand(50, 7) * 1e-8
        mae = np.mean(np.abs(p_curr - p_prev))
        assert mae < threshold

        # Non-converged case
        p_bad = p_prev + np.random.rand(50, 7) * 0.1
        mae_bad = np.mean(np.abs(p_bad - p_prev))
