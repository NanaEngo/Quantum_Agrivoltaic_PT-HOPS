    ...
    if cell["cell_type"] == "code":
            ...
            if "import pandas as pd" not in "".join(source):
                for i, line in enumerate(source):
                    if "import numpy as np" in line:
                        ...
                        source.insert(
                            i + 2, f"{indent}os.makedirs(DATA_DIR, exist_ok=True)\n"
                        )
                        break
            cell["source"] = source
            continue
        ...
        if any("FMO_Site_Energies.pdf" in line for line in source):
            for line in source:
                ...
                if "plt.savefig" in line and "fmo_hamiltonian_data.csv" not in "".join(
                    source
                ):
                    new_source.append(
        ...
        if any(
            "PAR_Transmission__Clean_vs_Dusty_Conditions.pdf" in line for line in source
        ):
            for line in source:
                indent = get_indent(line)
                ...
                if (
                    "plt.plot(wavelengths, T_dusty, linewidth=2," in line
                ...
                if "plt.savefig" in line and "solar_irradiance_data.csv" not in "".join(
                    source
                ):
                    new_source.append(
