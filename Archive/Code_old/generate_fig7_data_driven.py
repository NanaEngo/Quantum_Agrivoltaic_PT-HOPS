...
def generate_figure_7():
    ...
    for w0, S in modes:
        gv = 10
        J_v = (
            S * w0**2 * gv * omega / ((omega**2 - w0**2) ** 2 + (gv * omega) ** 2) * 500
        )
    ...
    opv_t = (
        0.4
        + 0.4 * np.exp(-(((wl - 580) / 120) ** 2))
        + 0.2 * np.exp(-(((wl - 750) / 20) ** 2))
        + 0.2 * np.exp(-(((wl - 825) / 15) ** 2))
    )
    psu_r = (
        0.4 * np.exp(-(((wl - 680) / 40) ** 2))
        + 0.9 * np.exp(-(((wl - 750) / 25) ** 2))
        + 0.75 * np.exp(-(((wl - 825) / 12) ** 2))
    )
