...
def _dual_band_transmission(omega_cm, filter_cfg):
    ...
    """
    band_centers_nm = filter_cfg.get('band_centers_nm', [750.0, 820.0])
    bandwidth_cm    = float(filter_cfg.get('bandwidth_cm', 100.0))
    weights         = filter_cfg.get('weights', [1.0, 1.0])
...
def _build_initial_state_for_label(H, label, pulse_cfg, filter_cfg=None):
    ...
    if label == 'broadband':
        ...
        #   Δν [cm⁻¹] ≈ 14.7 / FWHM_t [fs]
        fwhm_t_fs = float(pulse_cfg.get('fwhm', 50.0))
        fwhm_spectral_cm = 14.7 / fwhm_t_fs          # spectral FWHM in cm⁻¹
        sigma_in = fwhm_spectral_cm / (2.0 * np.sqrt(2.0 * np.log(2.0)))
        center_freq = float(pulse_cfg.get('center_freq', 12500.0))  # cm⁻¹
...
def run_full_fmo_simulation(cfg):
    ...
    # FIX H-1: return filtered t_axis explicitly, not the stale inner-loop t_save
...
def _run_temperature_sweep(cfg, H, time_points):
    ...
    pulse_cfg = cfg.get('pulse', {})
    filter_cfg = cfg.get('spectral_filter', None)
    # Use a smaller trajectory count for the sweep to keep runtime manageable.
    # Override via cfg['simulation']['n_traj_temp_sweep'] (default: 10).
    n_traj_sweep = cfg.get('simulation', {}).get('n_traj_temp_sweep', 10)

    # FIX C-1 (also applied here): normalise vibronic_damping to flat array
    vib_freqs = bath.get('vibronic_frequencies', [])
    vib_hr = bath.get('huang_rhys_factors', [])
    vib_damping_raw = bath.get('vibronic_damping', 10.0)
    vib_damping = (np.array(vib_damping_raw, dtype=float)
        ...
        for _ in range(n_traj_sweep):
            ...
            for label in ['filtered', 'broadband']:
                ...
                try:
                    ...
                    # phi = fraction of population transferred away from site 1
                    phi = 1.0 - data['populations'][-1, 0]
                    eta_vals[label] = float(phi)
                except Exception as e:
                    logger.warning(f"  T={T}K {label} trajectory failed: {e}")
                    eta_vals[label] = None

            if eta_vals.get('filtered') is not None and eta_vals.get('broadband') is not None:
                denom = max(eta_vals['broadband'], 1e-6)
                eta_samples.append((eta_vals['filtered'] - eta_vals['broadband']) / denom)

        if eta_samples:
            eta_temp[t_idx] = float(np.mean(eta_samples))
            eta_temp_err[t_idx] = float(np.std(eta_samples)) if len(eta_samples) > 1 else 0.04
        else:
            logger.warning(f"  T={T}K: all trajectories failed — eta set to NaN")
            eta_temp[t_idx] = np.nan
            eta_temp_err[t_idx] = np.nan

        print(f"  T={T:.0f}K: η = {eta_temp[t_idx]:.4f} ± {eta_temp_err[t_idx]:.4f} "
              f"(n={len(eta_samples)})")
...
def _build_disorder_samples(cfg, H, time_points, n_samples=100, rng_seed=42):
    ...
    pulse_cfg = cfg.get('pulse', {})
    filter_cfg = cfg.get('spectral_filter', None)
    sigma_disorder = cfg.get('simulation', {}).get('disorder_sigma_cm', 50.0)  # cm⁻¹
    ...
    for i in _tqdm(range(n_samples), desc="Disorder Ensemble", unit="sample"):
        ...
        for label in ['filtered', 'broadband']:
            ...
            try:
                ...
                logger.warning(f"  Disorder sample {i} {label} failed: {e}")
                eta_vals[label] = None

        if eta_vals.get('filtered') is not None and eta_vals.get('broadband') is not None:
            denom = max(eta_vals['broadband'], 1e-6)
            disorder_samples.append((eta_vals['filtered'] - eta_vals['broadband']) / denom)

    if not disorder_samples:
    ...
    return np.array(disorder_samples)
...
def _generate_spectral_figure(cfg, output_dir):
    ...
    omega = wavenumbers * 2 * np.pi * c_cms  # rad/s

    lambda_dl_cm = float(bath.get('reorganization_energy', 35.0))
    gamma_dl_cm  = float(bath.get('drude_cutoff', 50.0))
    lambda_dl = lambda_dl_cm * 2 * np.pi * c_cms  # rad/s units
    gamma_dl  = gamma_dl_cm  * 2 * np.pi * c_cms
    ...
    for freq_cm, S_k, damp_cm in zip(vib_freqs_cm, vib_hr, vib_damp_cm):
        w_k   = freq_cm * 2 * np.pi * c_cms
        g_k   = damp_cm * 2 * np.pi * c_cms
        lam_k = S_k * freq_cm * 2 * np.pi * c_cms
...
def generate_figures(cfg, sim_results, time_points):
    ...
    try:
        ...
        n_disorder = cfg.get('simulation', {}).get('n_disorder_samples', 100)
