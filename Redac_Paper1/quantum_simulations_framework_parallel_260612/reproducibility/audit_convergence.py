...
def run_convergence_audit(cfg=None):
    ...
    for L in _tqdm(depths, desc="Hierarchy Sweep", unit="level"):
        ...
        sim_data = simulator.simulate_dynamics(
        ...
        from src.io.csv_storage import CSVDataStorage as _CSV
    ...
    if len(depths) >= 2:
        ...
        if max_diff_last < 1e-18:  # Truly zero/identical
            logger.error(
                f"FAKE DATA DETECTED: L={depths[-2]} and L={depths[-1]} populations are identical. "
    ...
    try:
        ...
        logger.info(
            f"Convergence audit complete. MAE({depths[0]}->{depths[1]})={diffs.get(depths[1], 0):.2e}, MAE({depths[1]}->{depths[2]})={diffs.get(depths[2], 0):.2e}"
        )
...
def run_detailed_balance_audit(cfg=None):
    ...
    # Use dt=1.0 fs for numerical stability (dt=10 fs causes NaN in MesoHOPS)
    t_long = DEFAULT_TIME_LONG  # fs
    time_points = np.arange(0, t_long, 1.0)
    init_state = np.zeros(H.shape[0], dtype=complex)
    init_state[0] = 1.0
