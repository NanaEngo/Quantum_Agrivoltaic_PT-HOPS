    ...
    for cell in notebook["cells"]:
        if cell["cell_type"] == "code":
            ...
            if "def calculate_qfi" in source and "1e-12" in source:
                ...
                source = re.sub(
                    r"if eigenvals\[i\] \+ eigenvals\[j\] > 1e-12:",
                    "if eigenvals[i] + eigenvals[j] > 1e-8:",
                    source,
                )
                ...
                if "return 2 * qfi" in source:
                    ...
                    for line in lines:
                        if (
                            "return 2 * qfi" in line
                            and line.strip() == "return 2 * qfi"
                        ):
                            new_lines.append(
                    ...
                    source = "\n".join(new_lines)
                print(
                    "Updated QFI calculation with more conservative threshold and upper bound"
                )

            # Fix 3: Complete the transmission_profiles list
            if (
                "transmission_profiles = [" in source
                and "# Run optimization to get optimized parameters" in source
            ):
