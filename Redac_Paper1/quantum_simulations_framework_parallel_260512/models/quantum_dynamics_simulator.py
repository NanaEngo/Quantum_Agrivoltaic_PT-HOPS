...
class QuantumDynamicsSimulator:
    ...
    def _get_memory_estimate(self) -> float:
        ...
        n_hierarchy_modes = len(self.gw_sysbath) if hasattr(self, 'gw_sysbath') and self.gw_sysbath else 189
    ...
    def simulate_dynamics(self, initial_state=None, time_points=None, dt_save=2.0, seeds=None, **kwargs):
        ...
        if HAS_PSUTIL:
            ...
            n_jobs = min(n_mem_jobs, cpu_count, 1)  # MAX_N_JOBS=1: 54 GB/traj on 128 GB RAM
            ...
            logger.warning(f"psutil not available; running sequentially (n_jobs=1)")
