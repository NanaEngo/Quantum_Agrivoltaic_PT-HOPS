...
for cell in nb["cells"]:
    if cell["cell_type"] == "code":
        ...
        if 'FIGURES_DIR = "Graphics/"' in "".join(source):
            ...
            for line in source:
                if (
                    "import pandas as pd" in line
                    or "DATA_DIR =" in line
                    or "os.makedirs(DATA_DIR" in line
                ):
                    continue
        ...
        if any("FMO_Site_Energies.pdf" in line for line in source):
            ...
            for line in source:
                if (
                    "plt.figure(figsize=" in line
                    or "plt.subplot(" in line
                    or "pd.DataFrame(fmo_" in line
                ):
                    continue
        ...
        skip_next = False
        for line in source:
            if "# Data export" in line or (
                "pd.DataFrame" in line and "DATA_DIR" in line
            ):
                continue
        ...
        for line in clean_source:
            ...
            if "plt.savefig" in line:
                ...
                if "Quantum_Advantage_in_Energy_Transfer.pdf" in line:
                    final_source.append(
                        f"{indent}pd.DataFrame({{'site': sites, 'pop_filtered': populations_filtered_avg[-1], 'pop_broad': populations_broad_avg[-1]}}).to_csv(os.path.join(DATA_DIR, \"energy_transfer_advantage_data.csv\"))\n"
                    )
                elif "Pareto_Front__PCE_vs_ETR_Trade_off.pdf" in line:
                    final_source.append(
                        f"{indent}pd.DataFrame({{'pce': results_df['pce'], 'etr': results_df['etr_mean']}}).to_csv(os.path.join(DATA_DIR, \"pareto_frontier_raw_data.csv\"))\n"
                    )
                elif "ETR_Uncertainty_Distribution.pdf" in line:
                    final_source.append(
                        f"{indent}pd.DataFrame({{'etr_samples': mc_results['etr']['samples']}}).to_csv(os.path.join(DATA_DIR, \"etr_uncertainty_samples.csv\"))\n"
                    )
