    ...
    if cell["cell_type"] == "code":
        ...
        if any("Quantum_Advantage_in_Energy_Transfer.pdf" in line for line in source):
            for line in source:
                ...
                if "plt.savefig" in line:
                    ...
                    new_source.append(
                        "    pd.DataFrame({'site': sites, 'pop_filtered': populations_filtered_avg[-1], 'pop_broad': populations_broad_avg[-1]}).to_csv(os.path.join(DATA_DIR, \"energy_transfer_advantage_data.csv\"))\n"
                    )
            cell["source"] = new_source
            continue
        ...
        if any("Pareto_Front__PCE_vs_ETR_Trade_off.pdf" in line for line in source):
            for line in source:
                ...
                if "plt.savefig" in line:
                    new_source.append(
                        "pd.DataFrame({'pce': results_df['pce'], 'etr': results_df['etr_mean']}).to_csv(os.path.join(DATA_DIR, \"pareto_frontier_raw_data.csv\"))\n"
                    )
            cell["source"] = new_source
            continue
        ...
        if any("ETR_Uncertainty_Distribution.pdf" in line for line in source):
            for line in source:
                ...
                if "plt.savefig" in line:
                    new_source.append(
                        "pd.DataFrame({'etr_samples': mc_results['etr']['samples']}).to_csv(os.path.join(DATA_DIR, \"etr_uncertainty_samples.csv\"))\n"
                    )
            cell["source"] = new_source
            continue
