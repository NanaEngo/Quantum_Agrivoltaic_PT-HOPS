...
class TestPhysicsValidation:
    ...
    def test_thermalization_limit(self, dimer_hamiltonian, t_long, config):
        ...
        psi0 = np.ones(dimer_hamiltonian.shape[0], dtype=complex) / np.sqrt(
            dimer_hamiltonian.shape[0]
        )
        results = sim.simulate_dynamics(t_long, psi0, strict_mode=True)
        ...
        assert final_pops[0] < final_pops[1], (
            "Higher energy site should be less populated"
        )
    ...
    def test_high_temperature_decoherence(self, dimer_hamiltonian):
        ...
        sim_high = HopsSimulator(
            dimer_hamiltonian, temperature=1000.0, use_mesohops=False
        )
