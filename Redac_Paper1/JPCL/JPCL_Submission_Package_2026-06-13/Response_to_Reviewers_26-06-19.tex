...
\textbf{Response:} The high-frequency oscillations previously masked the long-time limit. The production ensemble has been re-run with verified canonical parameters ($L=8$, $K=2$, $\lambda_D = \SI{35}{\per\centi\meter}$, 12 vibronic modes). Revised Fig.~3 shows the $l_1$-norm coherence decay envelope and population equilibration, confirming environmentally-induced decoherence. The coherence lifetime $\tau_c$ extracted from the exponential fit is reported in the revised caption.
...
\subsection*{Additional Robustness Analysis (Phase 2)}

In response to the reviewers' request for a thorough assessment of parameter sensitivity, we have performed comprehensive robustness sweeps (Phase 2). All sweeps use $L=7$, $K=2$, SBD=3, $\Delta t = \SI{0.5}{\femto\second}$, $N=10$ trajectories (except temperature: $N=5$), with the production $\lambda_D = \SI{35}{\per\centi\meter}$, $\gamma_D = \SI{50}{\per\centi\meter}$, 12 vibronic modes.

\textbf{Temperature dependence.} $\eta(T)$ decreases monotonically from $\num{0.543}$ at \SI{285}{\kelvin} to $\num{0.386}$ at \SI{295}{\kelvin} ($\Delta\eta/\Delta T \approx \SI{-0.015}{\per\kelvin}$). The negative slope confirms a coherent transport mechanism: thermal agitation destroys vibronic coherences faster at higher temperatures. This data is presented in SI Fig.~S6.

\textbf{Bath parameter sensitivity.} Varying $\lambda_D$ and $\gamma_D$ by $\pm\SI{20}{\percent}$ yields $\eta \in [\num{0.28}, \num{0.62}]$. The production parameters are near-optimal; even the minimum $\eta = \num{0.28}$ ($\gamma_D = \SI{60}{\per\centi\meter}$) remains positive. Results are tabulated in SI Table~S3 and visualized in SI Fig.~S7.

\textbf{Spectral filter sweep.} Seven filter configurations were tested at $N=10$:
\begin{itemize}
    \item Dual-band filters targeting vibronic resonances: $\eta = \num{0.56}$ (both \SIlist{770;820}{\nano\meter} and \SIlist{730;820}{\nano\meter}).
    \item Broader bandwidth (\SI{200}{\per\centi\meter}): $\eta = \num{0.65}$ (increased photon flux).
    \item Narrower bandwidth (\SI{50}{\per\centi\meter}): $\eta = \num{0.53}$.
    \item Single-band filters: $\eta = \num{0.53}$ (\SI{700}{\nano\meter}) and $\eta = \num{0.52}$ (\SI{850}{\nano\meter}) — pending final acquisition.
    \item Off-resonant control (\SIlist{750;800}{\nano\meter}): $\eta = \num{-0.96}$, confirming that enhancement requires vibronic resonance alignment.
\end{itemize}
These results are reported in SI Table~S4 and SI Fig.~S8.
