...
def test_agrivoltaic_coupling_model():
    ...
    logger.info(
        f"AgrivoltaicCouplingModel: n_opv={model.n_opv_sites}, n_psu={model.n_psu_sites}, n_total={model.n_total}"
    )
...
def test_environmental_factors():
    ...
    temp_effect = env.temperature_effects_model(
        np.array([DEFAULT_TEMPERATURE, DEFAULT_TEMPERATURE + 10]),
        base_efficiency=DEFAULT_PCE,
    )
