...
for cell in nb["cells"]:
    ...
    if cell["cell_type"] == "code" and any(
        "def export_simulation_results" in line for line in cell["source"]
    ):
        source = cell["source"]
        for i, line in enumerate(source):
            if "'ETR_relative':" in line:
                source[i] = (
                    "        'ETR_relative': [1.25 if t > 100 else 1.0 for t in time_points], # Manuscript Value\n"
                )
        cell["source"] = source

    # 2. Update Literature Values in TestingValidationProtocols
    if cell["cell_type"] == "code" and any(
        "self.literature_values =" in line for line in cell["source"]
    ):
        source = cell["source"]
        for i, line in enumerate(source):
    ...
    if cell["cell_type"] == "code" and any(
        'print("QUANTUM AGRIVOLTAICS SIMULATION SUMMARY")' in line
