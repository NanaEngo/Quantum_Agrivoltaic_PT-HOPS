...
print(
    f"  Peak coherence: {np.max(coherence):.4f} at {time_points[np.argmax(coherence)]:.1f} fs"
)
...
fig.suptitle(
    r"Quantum Metrics Evolution in FMO Complex (PT-HOPS+LTC, $T$ = 295 K)",
    fontsize=14,
    fontweight="bold",
    y=0.98,
)
...
(l1,) = ax.plot(
    time_points, purity, color="#2980b9", linewidth=2, label=r"Purity Tr($\rho^2$)"
)
