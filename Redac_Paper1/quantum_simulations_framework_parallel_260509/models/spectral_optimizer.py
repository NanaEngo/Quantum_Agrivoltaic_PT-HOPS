...
class SpectralOptimizer:
    ...
    def calculate_opv_efficiency(self, transmission: np.ndarray) -> float:
        ...
        # Calculate efficiency (realistic maximum for OPV)
        if total_power > 0:
            efficiency = min(OPV_MAX_EFFICIENCY, OPV_UTILIZATION_FACTOR * absorbed_power / total_power)
        else:
            efficiency = 0.0
    ...
    def calculate_psu_efficiency(self, transmission: np.ndarray) -> float:
        ...
        # Calculate efficiency (realistic maximum for PSU)
        if total_power > 0:
            efficiency = min(PSU_MAX_EFFICIENCY, PSU_UTILIZATION_FACTOR * absorbed_power / total_power)
        else:
            efficiency = PSU_MIN_EFFICIENCY  # Minimum efficiency
    ...
    def optimize_spectral_splitting(
        ...
        for _i in range(n_filters):
            ...
            # Width (nm)
            bounds.append((FILTER_WIDTH_MIN, FILTER_WIDTH_MAX))
    ...
    def _find_best_simple_transmission(self) -> np.ndarray:
        ...
        if self.w_pce > self.w_etr:
            ...
            opv_peak_regions = (self.lambda_range > OPV_ACTIVE_REGION_MIN) & (self.lambda_range < 500)
...
if __name__ == "__main__":
    ...
    lambda_range = np.linspace(SOLAR_LAMBDA_MIN, SOLAR_LAMBDA_MAX, 801)
    ...
    for i, lam in enumerate(lambda_range):
        if OPV_ACTIVE_REGION_MIN <= lam <= OPV_ACTIVE_REGION_MAX:  # OPV active region
