...
def analyze_fmo_hamiltonian_data():
    ...
    try:
        ...
        plt.savefig(
            FIGURES_DIR / "fmo_hamiltonian_analysis.pdf", bbox_inches="tight", dpi=300
        )
        plt.savefig(
            FIGURES_DIR / "fmo_hamiltonian_analysis.png", bbox_inches="tight", dpi=300
        )
        plt.close()
...
def analyze_fmo_site_energies():
    ...
    try:
        ...
        print(df.head())
        print(
            f"Energy range: {df['Energy_cm-1'].min():.0f} to {df['Energy_cm-1'].max():.0f} cm⁻¹"
        )
        ...
        plt.grid(True)
        plt.savefig(
            FIGURES_DIR / "fmo_site_energies_analysis.pdf", bbox_inches="tight", dpi=300
        )
        plt.savefig(
            FIGURES_DIR / "fmo_site_energies_analysis.png", bbox_inches="tight", dpi=300
        )
        plt.close()
...
def analyze_quantum_dynamics():
    ...
    try:
        ...
        metrics_df = pd.read_csv(
            DATA_DIR / "fmo_dynamics_quantum_metrics.csv", index_col=0
        )
        ...
        axes[1, 1].plot(
            metrics_df.index, metrics_df["Entropy"], label="Entropy", linewidth=1.5
        )
        axes[1, 1].plot(
            metrics_df.index, 1 - metrics_df["Purity"], label="1-Purity", linewidth=1.5
        )
        ...
        plt.tight_layout()
        plt.savefig(
            FIGURES_DIR / "quantum_dynamics_analysis.pdf", bbox_inches="tight", dpi=300
        )
        plt.savefig(
            FIGURES_DIR / "quantum_dynamics_analysis.png", bbox_inches="tight", dpi=300
        )
        plt.close()
...
def analyze_eco_design():
    ...
    try:
        ...
        if not bio_df.empty:
            ...
            scores = (
                bio_df["biodegradability_score"]
                if "biodegradability_score" in bio_df.columns
                else [0] * len(bio_df)
            )
        ...
        plt.tight_layout()
        plt.savefig(
            FIGURES_DIR / "eco_design_analysis.pdf", bbox_inches="tight", dpi=300
        )
        plt.savefig(
            FIGURES_DIR / "eco_design_analysis.png", bbox_inches="tight", dpi=300
        )
        plt.close()
...
def analyze_solar_spectrum():
    ...
    try:
        ...
        plt.grid(True)
        plt.savefig(
            FIGURES_DIR / "solar_spectrum_analysis.pdf", bbox_inches="tight", dpi=300
        )
        plt.savefig(
            FIGURES_DIR / "solar_spectrum_analysis.png", bbox_inches="tight", dpi=300
        )
        plt.close()
