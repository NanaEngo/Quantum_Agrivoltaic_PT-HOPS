...
class AgrivoltaicCouplingModel:
    ...
    def _calculate_opv_response(self):
        ...
        for i, lam in enumerate(self.lambda_range):
            if lam > lambda_cutoff:
                ...
                R_opv[i] = self.opv_absorption_coeff * np.exp(
                    -(lam - lambda_cutoff) / 100
                )
            else:
    ...
    def calculate_opv_efficiency(self, transmission_func, voltage=0.8):
        ...
        input_power = (
            trapezoid(self.solar_spec, self.lambda_range) / 1000.0
        )  # W/m² to normalize

        # Calculate efficiency
        pce = (voltage * j_sc * fill_factor) / (
            input_power * 1000.0
    ...
    def calculate_opv_transmission(
        self, omega, peak_pos=1.8, peak_width=0.2, max_trans=0.7
    ):
        """
